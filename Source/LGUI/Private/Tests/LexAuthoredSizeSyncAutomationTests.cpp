...
/*
 * A panel measures an Auto child from ULexPanelSlot's authored snapshot rather than from the child's
 ...
 * ULexSpriteBase::SetSprite - which sizes the widget from the new art - left the row at the old art's size.
 *
...
namespace LexAuthoredSizeSyncTestLocal
{
	...
	struct FVerticalStackFixture
	{
		...
		ULexWidget* First = nullptr;
		ULexWidget* Second = nullptr;

		bool Build(UWorld* World)
		{
			Root = NewObject<ULexWidget>(World);
			First = NewObject<ULexWidget>(Root);
			Second = NewObject<ULexWidget>(Root);
			...
			ULexLayoutContainerStackBox* Stack =
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexAuthoredSizeSurvivesRuntimeResizeTest,
	"LGUI.Layout.AuthoredSize.RuntimeResizeSurvivesTheNextPass",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexAuthoredSizeSurvivesRuntimeResizeTest::RunTest(const FString& Parameters)
{
	using namespace LexAuthoredSizeSyncTestLocal;
	...
	// A gameplay-side resize on the measured axis. This is the exact shape of ULexSpriteBase::SetSprite
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexAuthoredSizeIgnoresLayoutOutputTest,
	"LGUI.Layout.AuthoredSize.LayoutOutputDoesNotBecomeAuthored",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexAuthoredSizeIgnoresLayoutOutputTest::RunTest(const FString& Parameters)
{
	using namespace LexAuthoredSizeSyncTestLocal;
	FScopedTestWorld TestWorld;
	ULexUIManagerWorldSubsystem* Manager = ULexUIManagerWorldSubsystem::GetInstance(TestWorld.World);
	...
	ULexPanelSlot* Slot = Fixture.First->GetPanelSlot();
