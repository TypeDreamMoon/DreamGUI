...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexTextMarginInsetsEachEdgeTest,
	"LGUI.Text.Margin.EachEdgeIsInsetByItsOwnValue",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexTextMarginInsetsEachEdgeTest::RunTest(const FString& Parameters)
{
	using namespace LexTextBoxTestLocal;
	...
	ULexText::GetContentBox(Size, Pivot, FMargin(40.0f, 0.0f, 0.0f, 0.0f), OutSize, OutPivot);
	...
	ULexText::GetContentBox(Size, Pivot, FMargin(0.0f, 30.0f, 0.0f, 0.0f), OutSize, OutPivot);
	...
	ULexText::GetContentBox(Size, Pivot, FMargin(10.0f, 10.0f, 10.0f, 10.0f), OutSize, OutPivot);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexTextMarginHonoursPivotTest,
	"LGUI.Text.Margin.TheInsetIsMeasuredFromTheWidgetNotThePivot",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexTextMarginHonoursPivotTest::RunTest(const FString& Parameters)
{
	using namespace LexTextBoxTestLocal;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexTextMarginDegenerateTest,
	"LGUI.Text.Margin.AMarginWiderThanTheWidgetLeavesNoRoomRatherThanNegativeRoom",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexTextMarginDegenerateTest::RunTest(const FString& Parameters)
{
	// An inside-out box would wrap at a negative width, which the layout has no sane reading of.
	FVector2f OutSize, OutPivot;
	ULexText::GetContentBox(FVector2f(100.0f, 50.0f), FVector2f(0.5f, 0.5f),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexTextBestFitPicksTheLargestThatFitsTest,
	"LGUI.Text.BestFit.PicksTheLargestSizeThatStillFits",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexTextBestFitPicksTheLargestThatFitsTest::RunTest(const FString& Parameters)
{
	...
	const float Chosen = ULexText::FindBestFitFontSize(FVector2f(1000.0f, 101.0f), 8.0f, 200.0f, Measure);
	...
	TestEqual(TEXT("whichever axis runs out first is the one that decides"),
		ULexText::FindBestFitFontSize(FVector2f(100.0f, 10000.0f), 8.0f, 200.0f, Measure), 25.0f, 0.001f);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexTextBestFitBoundsTest,
	"LGUI.Text.BestFit.StaysWithinItsBoundsAtBothEnds",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexTextBestFitBoundsTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("a box with room to spare still draws at the authored size"),
		ULexText::FindBestFitFontSize(FVector2f(9999.0f, 9999.0f), 8.0f, 32.0f, Measure), 32.0f, 0.001f);
	...
	TestEqual(TEXT("a box too small for even the minimum stops at the minimum"),
		ULexText::FindBestFitFontSize(FVector2f(1.0f, 1.0f), 8.0f, 32.0f, Measure), 8.0f, 0.001f);

	// A degenerate range must not bisect into nonsense.
	TestEqual(TEXT("a minimum above the maximum collapses to the minimum"),
		ULexText::FindBestFitFontSize(FVector2f(9999.0f, 9999.0f), 40.0f, 32.0f, Measure), 40.0f, 0.001f);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexTextBestFitCheapWhenItFitsTest,
	"LGUI.Text.BestFit.CostsOneMeasurementWhenTheTextAlreadyFits",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexTextBestFitCheapWhenItFitsTest::RunTest(const FString& Parameters)
{
	...
	ULexText::FindBestFitFontSize(FVector2f(9999.0f, 9999.0f), 8.0f, 200.0f, Measure);
