...
namespace LexNameReleaseTestLocal
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexReleaseNameFreesItTest,
	"LGUI.Prefab.NameRelease.TheNameIsActuallyFreedAfterwards",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexReleaseNameFreesItTest::RunTest(const FString& Parameters)
{
	using namespace LexNameReleaseTestLocal;
	using namespace LEXUIPREFAB_SERIALIZER_NEWEST_NAMESPACE;
	FScopedGameWorld TestWorld;

	ULexWidget* Outer = NewObject<ULexWidget>(TestWorld.World, TEXT("ReleaseOuter"), RF_Public | RF_Transactional);
	const FName ContestedName(TEXT("Contested"));
	UObject* Squatter = NewObject<ULexPanelSlot>(Outer, ContestedName);
	...
	UObject* Replacement = NewObject<ULexPanelSlot>(Outer, ContestedName);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexReleaseNameRefusesDyingTest,
	"LGUI.Prefab.NameRelease.RefusesAnObjectAlreadyBeingDestroyed",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexReleaseNameRefusesDyingTest::RunTest(const FString& Parameters)
{
	using namespace LexNameReleaseTestLocal;
	using namespace LEXUIPREFAB_SERIALIZER_NEWEST_NAMESPACE;
	FScopedGameWorld TestWorld;

	ULexWidget* Outer = NewObject<ULexWidget>(TestWorld.World, TEXT("RefuseOuter"), RF_Public | RF_Transactional);
	UObject* Dying = NewObject<ULexPanelSlot>(Outer, TEXT("Dying"));
