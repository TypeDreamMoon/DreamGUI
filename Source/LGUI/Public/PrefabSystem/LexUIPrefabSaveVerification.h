...
namespace LexUIPrefabSystem
{
	 ...
	 * only escalate to a save failure by opt-in (ULexUISettings::bBlockPrefabSaveOnPropertyDrift).
	 */
	struct LGUI_API FLexUIPrefabSaveVerificationResult
	{
	 ...
	 */
	LGUI_API FLexUIPrefabSaveVerificationResult VerifyPrefabSaveRoundTrip(ULexUIPrefab* InPrefab, ULexWidget* InSourceRoot);
	 ...
	 */
	struct LGUI_API FLexUIPrefabEditorPayloadSnapshot
	{
		void Capture(const ULexUIPrefab* InPrefab);
		void Restore(ULexUIPrefab* InPrefab) const;
