						...
						if (auto generatedClass = blueprint->GeneratedClass)
						{
							if (auto objectInstance = generatedClass->GetDefaultObject())
							{
								...
								for (const auto propertyItem : propertyField)
								{
									...
									{
										if (structProperty->Struct == FDreamUIComponentReference::StaticStruct())
										{
											if (structProperty->GetFName() == propertyName)
											{
												FDreamUIComponentReference* structPtr = structProperty->ContainerPtrToValuePtr<FDreamUIComponentReference>(objectInstance);
...
{
	GetSchema()->ReconstructNode(*(UK2Node_DreamGUICompRef_GetComponent*)this);
	return false;
}
