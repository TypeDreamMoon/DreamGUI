 ...
 * place — UMG's "Bind Widgets" panel adapted to DreamUI's code-behind trinity.
 *
 ...
 * - Selected Widget Events: every FDreamUIEventDelegate on the selected widget's behaviours with
 *   bound status and one-click handler generation (the Event+ flow), plus promote-to-variable.
 */
class SDreamUIPrefabBehaviourViewer : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SDreamUIPrefabBehaviourViewer) {}
	SLATE_END_ARGS()
...
private:
	...
	void RunAutoBind();
	void BuildWidgetReferenceSection(UClass* BehaviourClass, UDreamUIBehaviour* Primary);
