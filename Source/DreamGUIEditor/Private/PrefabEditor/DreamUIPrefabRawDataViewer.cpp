...
#define LOCTEXT_NAMESPACE "DreamGUIPrefabRawDataViewer"

namespace DreamUIPrefabRawDataViewerLocal
{
	FString PrefabVersionToString(uint16 Version)
	{
		const TCHAR* Name = TEXT("Unknown");
		switch ((EDreamUIPrefabVersion)Version)
		{
		case EDreamUIPrefabVersion::OldVersion: Name = TEXT("OldVersion"); break;
		case EDreamUIPrefabVersion::BuiltinFArchive: Name = TEXT("BuiltinFArchive"); break;
		case EDreamUIPrefabVersion::NestedDefaultSubObject: Name = TEXT("NestedDefaultSubObject"); break;
		case EDreamUIPrefabVersion::ObjectName: Name = TEXT("ObjectName"); break;
		case EDreamUIPrefabVersion::CommonActor: Name = TEXT("CommonActor"); break;
		case EDreamUIPrefabVersion::ActorAttachToSubPrefab: Name = TEXT("ActorAttachToSubPrefab"); break;
		case EDreamUIPrefabVersion::NewObjectOnNestedPrefab: Name = TEXT("NewObjectOnNestedPrefab"); break;
		case EDreamUIPrefabVersion::FTextAsReference: Name = TEXT("FTextAsReference"); break;
		default: break;
		}
		const uint16 Newest = (uint16)EDreamUIPrefabVersion::NEWEST;
	...
	{
		const TCHAR* Name = TEXT("Unknown");
		switch ((EDreamUIPrefabSchemaVersion)Version)
		{
		case EDreamUIPrefabSchemaVersion::Unversioned: Name = TEXT("Unversioned"); break;
		case EDreamUIPrefabSchemaVersion::PanelSlotOwnership: Name = TEXT("PanelSlotOwnership"); break;
		default: break;
		}
		const uint16 Newest = (uint16)EDreamUIPrefabSchemaVersion::NEWEST;
		return FString::Printf(TEXT("%d — %s%s"), Version, Name,
			Version == Newest ? TEXT(" (newest)") : *FString::Printf(TEXT(" (newest is %d)"), Newest));
	}
	...
	int32 CountWidgets(UDreamWidget* Widget)
	{
		if (!IsValid(Widget))
		{
			return 0;
...
void SDreamUIPrefabRawDataViewer::Construct(const FArguments& InArgs, TSharedPtr<FDreamUIPrefabEditor> InPrefabEditorPtr, UObject* InObject)
{
	...
	[
		...
		[
			...
			.ColorAndOpacity(FSlateColor(DreamUIPrefabRawDataViewerLocal::DimColor))
		]
		+ SVerticalBox::Slot()
		.FillHeight(1.0f)
		[
...
bool SDreamUIPrefabRawDataViewer::MatchesFilter(const FString& Haystack) const
{
	return FilterString.IsEmpty() || Haystack.Contains(FilterString);
}

void SDreamUIPrefabRawDataViewer::Rebuild()
{
	using namespace DreamUIPrefabRawDataViewerLocal;
...
TSharedRef<SWidget> SDreamUIPrefabRawDataViewer::BuildOverviewSection(UDreamUIPrefab* Prefab)
{
	using namespace DreamUIPrefabRawDataViewerLocal;
...
TSharedRef<SWidget> SDreamUIPrefabRawDataViewer::BuildReferenceListsSection(UDreamUIPrefab* Prefab)
{
	using namespace DreamUIPrefabRawDataViewerLocal;
	TSharedRef<SVerticalBox> Box = SNew(SVerticalBox);
...
TSharedRef<SWidget> SDreamUIPrefabRawDataViewer::BuildGuidMapSection(UDreamUIPrefab* Prefab)
{
	using namespace DreamUIPrefabRawDataViewerLocal;
	TSharedRef<SVerticalBox> Box = SNew(SVerticalBox);
	UDreamUIPrefabHelperObject* Helper = Prefab->GetPrefabHelperObject();
	...
	for (const TPair<FGuid, TObjectPtr<UObject>>& Pair : Helper->MapGuidToObject)
	{
		...
		if (bValid)
		{
			const UDreamWidget* AsWidget = Cast<UDreamWidget>(Object);
...
TSharedRef<SWidget> SDreamUIPrefabRawDataViewer::BuildSubPrefabSection(UDreamUIPrefab* Prefab)
{
	using namespace DreamUIPrefabRawDataViewerLocal;
	TSharedRef<SVerticalBox> Box = SNew(SVerticalBox);
	UDreamUIPrefabHelperObject* Helper = Prefab->GetPrefabHelperObject();
	...
	{
		const UDreamWidget* RootWidget = Pair.Key.Get();
