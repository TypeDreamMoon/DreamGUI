...
void FDreamUIPrefabEditorCommand::RegisterCommands()
{
	...
	UI_COMMAND(RawDataViewer, "Raw Data", "Open the raw data panel: the serialized objects and properties of this prefab, as stored in the asset.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(BehaviourViewer, "Behaviour", "Open the Behaviour panel: widget references with quick bind, provided events and functions, and per-widget event handler shortcuts.", EUserInterfaceActionType::Button, FInputChord());
	...
	UI_COMMAND(OpenPrefabHelperObject, "Prefab Helper Object", "Open the PrefabHelperObject of this prefab in its own details window: the GUID map, sub-prefab records and override parameters the editor keeps for it.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(OpenBehaviourBlueprint, "Behaviour BP", "Open this prefab's companion behaviour blueprint (a UDreamUIBehaviour script on the root widget), creating BP_<PrefabName> next to the prefab if there is none yet. Like a UMG Widget Blueprint's Graph.", EUserInterfaceActionType::Button, FInputChord());
