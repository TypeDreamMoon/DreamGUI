...
#define LOCTEXT_NAMESPACE "DreamGUIPrefabBehaviourViewer"

namespace DreamUIPrefabBehaviourViewerLocal
{
	...
	FString DescribeBoundValue(UObject* Value)
	{
		...
		if (const UDreamWidget* AsWidget = Cast<UDreamWidget>(Value))
		{
			return AsWidget->GetDisplayName();
		}
		if (const UDreamWidget* Owner = Value->GetTypedOuter<UDreamWidget>())
		{
...
void SDreamUIPrefabBehaviourViewer::Construct(const FArguments& InArgs, TSharedPtr<FDreamUIPrefabEditor> InPrefabEditorPtr, UObject* InObject)
{
	using namespace DreamUIPrefabBehaviourViewerLocal;
	PrefabEditorPtr = InPrefabEditorPtr;
	PrefabWeak = Cast<UDreamUIPrefab>(InObject);

	if (InPrefabEditorPtr.IsValid())
	{
		InPrefabEditorPtr->OnSelectionChanged.AddSP(this, &SDreamUIPrefabBehaviourViewer::HandleSelectionChanged);
	}
...
bool SDreamUIPrefabBehaviourViewer::MatchesFilter(const FString& Haystack) const
{
	return FilterString.IsEmpty() || Haystack.Contains(FilterString);
}

void SDreamUIPrefabBehaviourViewer::HandleSelectionChanged()
{
	Rebuild();
}

void SDreamUIPrefabBehaviourViewer::RunAutoBind()
{
	using namespace DreamUIPrefabBehaviourViewerLocal;
	TSharedPtr<FDreamUIPrefabEditor> Editor = PrefabEditorPtr.Pin();
	UDreamUIPrefab* Prefab = PrefabWeak.Get();
	UDreamWidget* RootWidget = Editor.IsValid() ? Editor->GetLoadedRootWidget() : nullptr;
	...
	DreamUIPrefabBehaviourUtils::AutoBindAndValidate(RootWidget, Prefab, BoundDetails, Problems, /*bPerformAutoBind*/true);
...
void SDreamUIPrefabBehaviourViewer::CollectBindCandidates(FObjectProperty* Property, TArray<TPair<UObject*, FString>>& OutCandidates, int32& OutSubPrefabSkipped) const
{
	...
	DreamUIPrefabBehaviourUtils::CollectSubPrefabWidgets(PrefabWeak.Get(), SubPrefabWidgets);
	UClass* TargetClass = Property->PropertyClass;
	TArray<UDreamWidget*> Stack{ RootWidget };
	while (Stack.Num() > 0)
	{
		...
		{
			if (UDreamVisual* Visual = Widget->GetVisual(); IsValid(Visual) && Visual->IsA(TargetClass))
			{
				Candidate = Visual;
				Label = FString::Printf(TEXT("%s · %s"), *Widget->GetDisplayName(), *Visual->GetClass()->GetName());
			}
		}
		else if (TargetClass->IsChildOf(UDreamUIBehaviour::StaticClass()))
		{
			if (UDreamUIBehaviour* Behaviour = Widget->GetComponent(TargetClass))
			{
				Candidate = Behaviour;
...
void SDreamUIPrefabBehaviourViewer::BuildWidgetReferenceSection(UClass* BehaviourClass, UDreamUIBehaviour* Primary)
{
	using namespace DreamUIPrefabBehaviourViewerLocal;
	...
	for (TFieldIterator<FObjectProperty> It(BehaviourClass, EFieldIteratorFlags::IncludeSuper); It; ++It)
	{
		...
		if (Property->GetOwnerClass() == UDreamUIBehaviour::StaticClass())
		{
			continue;
		}
		Total++;
		...
		{
			const TWeakObjectPtr<UDreamUIBehaviour> PrimaryWeak = Primary;
			Row->AddSlot()
			.AutoWidth()
			.VAlign(VAlign_Center)
			.Padding(4, 1)
			[
				...
				.OnGetMenuContent_Lambda([this, Property, PrimaryWeak]()
				{
					...
					{
						UDreamUIBehaviour* LocalPrimary = PrimaryWeak.Get();
						...
						if (UDreamUIPrefab* LocalPrefab = PrefabWeak.Get())
						{
							if (UDreamUIPrefabHelperObject* Helper = LocalPrefab->GetPrefabHelperObject())
							{
								Helper->Modify();
								Helper->SetAnythingDirty();
							}
	...
	if (Total == 0)
	{
		...
		[
			...
			.Font(IDetailLayoutBuilder::GetDetailFont())
			.ColorAndOpacity(FSlateColor(DreamUIPrefabBehaviourViewerLocal::DimColor))
			.AutoWrapText(true)
		];
	}
...
void SDreamUIPrefabBehaviourViewer::BuildProvidesSection(UClass* BehaviourClass)
{
	using namespace DreamUIPrefabBehaviourViewerLocal;
	TSharedRef<SVerticalBox> Rows = SNew(SVerticalBox);
	int32 Shown = 0;
	...
	auto AddRow = [this, &Rows, &Shown, &Total](const TCHAR* Kind, const FLinearColor& KindColor, const FString& Signature, const FString& OwnerName)
	{
		...
		[
			DreamUIPrefabBehaviourViewerLocal::MakeStripedRow(Shown,
				SNew(SHorizontalBox)
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.VAlign(VAlign_Center)
				...
				[
					...
					.Font(DreamUIPrefabBehaviourViewerLocal::MonoFont())
					.ColorAndOpacity(FSlateColor(DreamUIPrefabBehaviourViewerLocal::ValueColor))
				]
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.VAlign(VAlign_Center)
	...
	for (TFieldIterator<UFunction> It(BehaviourClass, EFieldIteratorFlags::IncludeSuper); It; ++It)
	{
		...
		if (!Owner || !Owner->IsChildOf(UDreamUIBehaviour::StaticClass()))
		{
			continue;
		}
...
void SDreamUIPrefabBehaviourViewer::BuildSelectedWidgetSection()
{
	using namespace DreamUIPrefabBehaviourViewerLocal;
	TSharedPtr<FDreamUIPrefabEditor> Editor = PrefabEditorPtr.Pin();
	if (!Editor.IsValid())
	...
	UDreamWidget* SelectedWidget = Editor->GetSelectedWidgets().Num() > 0 ? Editor->GetSelectedWidgets()[0].Get() : nullptr;
	TSharedRef<SVerticalBox> Rows = SNew(SVerticalBox);
	...
	const TWeakObjectPtr<UDreamWidget> SelectedWeak = SelectedWidget;
	...
	[
		...
		[
			...
			{
				if (TSharedPtr<FDreamUIPrefabEditor> LocalEditor = PrefabEditorPtr.Pin(); LocalEditor.IsValid() && SelectedWeak.IsValid())
				{
	...
	DreamUIPrefabBehaviourUtils::DiscoverEvents(SelectedWidget, Events);
	int32 Shown = 0;
	for (const DreamUIPrefabBehaviourUtils::FDiscoveredEvent& Event : Events)
	{
		...
		for (EDreamUIBehaviourHandlerType HandlerType : { EDreamUIBehaviourHandlerType::Function, EDreamUIBehaviourHandlerType::Event })
		{
			const FText Label = HandlerType == EDreamUIBehaviourHandlerType::Function
			...
			[
				...
				{
					if (TSharedPtr<FDreamUIPrefabEditor> LocalEditor = PrefabEditorPtr.Pin())
					{
...
void SDreamUIPrefabBehaviourViewer::Rebuild()
{
	using namespace DreamUIPrefabBehaviourViewerLocal;
	...
	UDreamUIBehaviour* Primary = Editor->GetPrimaryBehaviour();
