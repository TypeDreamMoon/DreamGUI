...
public:
	DRAG_DROP_OPERATOR_TYPE(FHierarchyDreamWidgetDragDropOp, FDecoratedDragDropOp)
		...
		virtual ~FHierarchyDreamWidgetDragDropOp();
...
private:
	TOptional<EItemDropZone> HandleCanAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TWeakObjectPtr<UDreamWidget> TargetItem);
	FReply HandleAcceptDrop(FDragDropEvent const& DragDropEvent, EItemDropZone DropZone, TWeakObjectPtr<UDreamWidget> TargetItem);
	...
	bool SupportDrop(UDreamWidget* Dragging, UDreamWidget* Current, EItemDropZone DropZone);
