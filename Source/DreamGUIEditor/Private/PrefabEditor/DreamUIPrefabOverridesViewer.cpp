...
#define LOCTEXT_NAMESPACE "DreamGUIPrefabOverridesViewer"

namespace DreamUIPrefabOverridesViewerLocal
{
	...
	FString DescribeOverrideObject(UObject* Object)
	{
		...
		if (const UDreamWidget* AsWidget = Cast<UDreamWidget>(Object))
		{
			return FString::Printf(TEXT("%s · %s"), *AsWidget->GetDisplayName(), *Object->GetClass()->GetName());
		}
		if (const UDreamWidget* OwnerWidget = Object->GetTypedOuter<UDreamWidget>())
		{
...
void SDreamUIPrefabOverridesViewer::Construct(const FArguments& InArgs, TSharedPtr<FDreamUIPrefabEditor> InPrefabEditorPtr, UObject* InObject)
{
	using namespace DreamUIPrefabOverridesViewerLocal;
					...
					.Text_Lambda([this]()
					{
						...
						return Object
							? FText::Format(LOCTEXT("SelectedObjectFmt", "Editing: {0} — changes on a nested instance are recorded as overrides."),
								FText::FromString(DreamUIPrefabOverridesViewerLocal::DescribeOverrideObject(Object)))
...
bool SDreamUIPrefabOverridesViewer::MatchesFilter(const FString& Haystack) const
{
	return FilterString.IsEmpty() || Haystack.Contains(FilterString);
}

UDreamWidget* SDreamUIPrefabOverridesViewer::ResolveOwnerWidget(UObject* Object)
{
	if (!IsValid(Object))
	{
		return nullptr;
	}
	if (UDreamWidget* AsWidget = Cast<UDreamWidget>(Object))
	{
		return AsWidget;
	}
	if (UDreamWidget* OuterWidget = Object->GetTypedOuter<UDreamWidget>())
	{
		return OuterWidget;
	}
...
void SDreamUIPrefabOverridesViewer::SelectOverrideObject(UObject* Object)
{
	...
	if (UDreamWidget* Widget = ResolveOwnerWidget(Object))
	{
		if (TSharedPtr<FDreamUIPrefabEditor> Editor = PrefabEditorPtr.Pin())
		{
			Editor->SelectWidgets(TSet<UDreamWidget*>{ Widget }, false);
		}
	}
}

void SDreamUIPrefabOverridesViewer::RevertOverridesOnObject(UDreamUIPrefabHelperObject* InHelper, UObject* InObject, const TArray<FName>& InPropertyNames)
{
...
void SDreamUIPrefabOverridesViewer::Rebuild()
{
	using namespace DreamUIPrefabOverridesViewerLocal;
	...
	{
		for (const FDreamUIPrefabOverrideParameterData& Override : Pair.Value.ObjectOverrideParameterArray)
		{
			TotalObjects++;
	...
	for (const TPair<TObjectPtr<UDreamWidget>, FDreamUISubPrefabData>& Pair : Helper->SubPrefabMap)
	{
		UDreamWidget* InstanceRoot = Pair.Key.Get();
		...
		{
			const FDreamUIPrefabOverrideParameterData& Override = Data.ObjectOverrideParameterArray[Index];
						...
						.OnGetMenuContent_Lambda([this, ObjectWeak, Names = Override.MemberPropertyNames]()
						{
							...
							for (const FName& PropertyName : Names)
							{
								MenuBuilder.AddMenuEntry(
									...
									{
										UDreamUIPrefab* LocalPrefab = PrefabWeak.Get();
		...
		[
			...
			[
				...
				[
					...
					.OnClicked_Lambda([this, RootWeak = TWeakObjectPtr<UDreamWidget>(InstanceRoot)]()
					{
						UDreamUIPrefab* LocalPrefab = PrefabWeak.Get();
						UDreamUIPrefabHelperObject* LocalHelper = IsValid(LocalPrefab) ? LocalPrefab->GetPrefabHelperObject() : nullptr;
