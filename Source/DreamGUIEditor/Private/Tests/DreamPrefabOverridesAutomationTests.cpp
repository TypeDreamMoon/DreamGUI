...
#include "PrefabEditor/DreamUIPrefabOverridesViewer.h"
...
namespace DreamPrefabOverridesTestLocal
{
	 ...
	 * sub-prefab asset: UDreamUIPrefab hands out a helper object only once its version says its
	 * binary is readable, which is what the serialize below is for.
	 */
	struct FSubPrefabFixture
	{
		FScopedTestWorld TestWorld;
		UDreamUIPrefab* SubPrefabAsset = nullptr;
		UDreamUIPrefabHelperObject* SubHelper = nullptr;
		UDreamUIPrefabHelperObject* Helper = nullptr;
		UDreamWidget* InstanceRoot = nullptr;
		UDreamWidget* InstanceChild = nullptr;
		UDreamWidget* SourceRoot = nullptr;
		UDreamWidget* SourceChild = nullptr;
		...
		FSubPrefabFixture()
		{
			SubPrefabAsset = NewObject<UDreamUIPrefab>();
			UDreamWidget* AssetRoot = MakeWidget(TEXT("AssetRoot"));
			AssetRoot->CreateNewLayoutContainer<UDreamLayoutContainerOverlay>();
			...
			bSerialized = DreamUIPrefabSystem::WidgetSerializer::SavePrefab(AssetRoot, SubPrefabAsset, ObjectToGuid, NoSubPrefabs, true);
			...
			Helper = NewObject<UDreamUIPrefabHelperObject>(GetTransientPackage(), NAME_None, RF_Transient | RF_Transactional);

			InstanceRoot = MakeWidget(TEXT("InstanceRoot"));
			InstanceRoot->CreateNewLayoutContainer<UDreamLayoutContainerOverlay>();
			...
			SourceRoot->CreateNewLayoutContainer<UDreamLayoutContainerOverlay>();
			...
			Data.AddMemberProperty(InstanceRoot, UDreamWidget::GetPropertyName_RelativeLocation());
			Data.AddMemberProperty(InstanceChild, UDreamWidget::GetPropertyName_DisplayName());
		...
		UDreamWidget* MakeWidget(const TCHAR* InDisplayName)
		{
		...
		{
			if (const FDreamUISubPrefabData* Data = Helper->SubPrefabMap.Find(InstanceRoot))
			{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamOverridesPerObjectRevertTest,
	"DreamGUI.Editor.PrefabOverrides.PerObjectRevertLeavesTheInstanceRootAlone",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamOverridesPerObjectRevertTest::RunTest(const FString& Parameters)
{
	using namespace DreamPrefabOverridesTestLocal;
	...
	SDreamUIPrefabOverridesViewer::RevertOverridesOnObject(
	...
	TestTrue(TEXT("the instance root is still pinned on its location"),
		RootNames.Contains(UDreamWidget::GetPropertyName_RelativeLocation()));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamOverridesWholeInstanceRevertTest,
	"DreamGUI.Editor.PrefabOverrides.WholeInstanceRevertDropsTheRootTransform",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamOverridesWholeInstanceRevertTest::RunTest(const FString& Parameters)
{
	using namespace DreamPrefabOverridesTestLocal;
	if (GEditor == nullptr || GEditor->Trans == nullptr)
	{
