...
#include "DreamImageBrushStructCustomization.h"
...
#define LOCTEXT_NAMESPACE "DreamImageBrushStructCustomization"
...
private:
	void OnBrushResourceChanged()
	{
		...
		if ( Result == FPropertyAccess::Success )
		{
			...
			{
				auto TemporaryBrush = static_cast<FDreamUIImageBrush*>(RawBrushData[BrushIndex]);
				if (TemporaryBrush)
				{
				}
			}

			using ImageSizeType = decltype(FDreamUIImageBrush::ImageSize);
			...
			using UVRegionType = decltype(FDreamUIImageBrush::UVRegion);
			...
			else if (auto DreamSprite = Cast<UDreamUISpriteData_BaseObject>(ResourceObject))
			{
				CachedTextureSize.X = DreamSprite->GetSpriteInfo().GetSourceWidth();
				CachedTextureSize.Y = DreamSprite->GetSpriteInfo().GetSourceHeight();
				CachedUVRegion = UVRegionType(0, 0, 1, 1);
			}
			else
			{
...
TSharedRef<IPropertyTypeCustomization> FDreamImageBrushStructCustomization::MakeInstance()
{
	return MakeShareable(new FDreamImageBrushStructCustomization);
}
void FDreamImageBrushStructCustomization::CustomizeHeader( TSharedRef<IPropertyHandle> StructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
	HeaderRow
	.NameContent()
	[
...
void FDreamImageBrushStructCustomization::CustomizeChildren( TSharedRef<IPropertyHandle> StructPropertyHandle, IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils )
{
	...
	.OverrideResetToDefault(FResetToDefaultOverride::Create(
		FIsResetToDefaultVisible::CreateSP(this, &FDreamImageBrushStructCustomization::IsImageSizeResetToDefaultVisible),
		FResetToDefaultHandler::CreateSP(this, &FDreamImageBrushStructCustomization::OnImageSizeResetToDefault),
	...
	.Visibility( TAttribute<EVisibility>::CreateSP(this, &FDreamImageBrushStructCustomization::GetMarginPropertyVisibility ) );
	StructBuilder.AddProperty(UVRegionProperty.ToSharedRef())
	.Visibility( TAttribute<EVisibility>::CreateSP(this, &FDreamImageBrushStructCustomization::GetUVRegionPropertyVisibility ) );
	StructBuilder.AddProperty(PixelsPerUnitMultiplierProperty.ToSharedRef())
	.Visibility(TAttribute<EVisibility>::CreateSP(this, &FDreamImageBrushStructCustomization::GetPixelsPerUnitMultiplierPropertyVisibility));
}

EVisibility FDreamImageBrushStructCustomization::GetMarginPropertyVisibility() const
{
	uint8 DrawAsType;
	...
	if (Cast<UDreamUISpriteData_BaseObject>(ResourceObject) != nullptr)
	{
...
EVisibility FDreamImageBrushStructCustomization::GetUVRegionPropertyVisibility()const
{
	UObject* ResourceObject = nullptr;
	FPropertyAccess::Result Result = ResourceObjectProperty->GetValue(ResourceObject);
...
EVisibility FDreamImageBrushStructCustomization::GetPixelsPerUnitMultiplierPropertyVisibility() const
{
	uint8 DrawAsType;
	FPropertyAccess::Result Result = DrawAsProperty->GetValue( DrawAsType );
...
bool FDreamImageBrushStructCustomization::IsImageSizeResetToDefaultVisible(TSharedPtr<IPropertyHandle> PropertyHandle) const
{
	UObject* ResourceObject;
...
void FDreamImageBrushStructCustomization::OnImageSizeResetToDefault(TSharedPtr<IPropertyHandle> PropertyHandle) const
{
	UObject* ResourceObject;
	if (FPropertyAccess::Success == ResourceObjectProperty->GetValue(ResourceObject) && ResourceObject)
	{
...
FVector2f FDreamImageBrushStructCustomization::GetDefaultImageSize() const
{
	...
	if (FPropertyAccess::Success == ResourceObjectProperty->GetValue(ResourceObject))
	{
		...
		{
			return FVector2f(DreamSprite->GetSpriteInfo().GetSourceWidth(), DreamSprite->GetSpriteInfo().GetSourceHeight());
		}
	}
