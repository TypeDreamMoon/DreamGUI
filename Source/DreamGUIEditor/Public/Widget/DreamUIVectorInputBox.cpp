...
#include "DreamUIVectorInputBox.h"
...
#define LOCTEXT_NAMESPACE "SDreamGUIVectorInputBox"

void SDreamUIVectorInputBox::Construct( const FArguments& InArgs )
{
...
void SDreamUIVectorInputBox::ConstructX( const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox )
{	
		...
		.ToolTipText(MakeAttributeLambda([=]
		{
			if (Value.Get().IsSet())
			{
				if (InArgs._EnableX.IsBound())
				{
					...
					: LOCTEXT("X_ToolTip_Control", "X Value Controlled by DreamGUI's Anchors");
				}
...
void SDreamUIVectorInputBox::ConstructY( const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox )
{
	TSharedRef<SWidget> LabelWidget = SNullWidget::NullWidget;
	if (InArgs._bColorAxisLabels)
	{
		...
		.ToolTipText(MakeAttributeLambda([=]
		{
			if (Value.Get().IsSet())
			{
				if (InArgs._EnableY.IsBound())
				{
					...
					: LOCTEXT("Y_ToolTip_Control", "Y Value Controlled by DreamGUI's Anchors");
				}
...
void SDreamUIVectorInputBox::ConstructZ( const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox )
{
	TSharedRef<SWidget> LabelWidget = SNullWidget::NullWidget;
	if (InArgs._bColorAxisLabels)
	{
		...
		.ToolTipText(MakeAttributeLambda([=]
		{
			if (Value.Get().IsSet())
			{
				if (InArgs._EnableZ.IsBound())
				{
					...
					: LOCTEXT("Z_ToolTip_Control", "Z Value Controlled by DreamGUI's Anchors");
				}
...
void SDreamUIVectorInputBox::ConstructW(const FArguments& InArgs, TSharedRef<SHorizontalBox> HorizontalBox)
{
	TSharedRef<SWidget> LabelWidget = SNullWidget::NullWidget;
	if (InArgs._bColorAxisLabels)
	{
		...
		.ToolTipText(MakeAttributeLambda([=]
		{
			if (Value.Get().IsSet())
			{
				if (InArgs._EnableW.IsBound())
				{
					...
					: LOCTEXT("W_ToolTip_Control", "W Value Controlled by DreamGUI's Anchors");
				}
