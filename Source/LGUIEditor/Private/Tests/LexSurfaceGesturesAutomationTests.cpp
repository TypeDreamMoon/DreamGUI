...
namespace LexSurfaceGestureTestLocal
{
	...
	FLexUIPrefabEditorViewportClient::FMoveDragTarget MakeTarget(const FVector2D& InStartPosition, const FVector& InTravel)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexSurfaceMoveDragKeepsSelectionRigidTest,
	"LGUI.Editor.SurfaceGestures.MoveDragSnapsTheSelectionOnceNotWidgetByWidget",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexSurfaceMoveDragKeepsSelectionRigidTest::RunTest(const FString& Parameters)
{
	using namespace LexSurfaceGestureTestLocal;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexSurfaceMoveDragGuideFollowsWritableAxesTest,
	"LGUI.Editor.SurfaceGestures.MoveDragClaimsAGuideOnlyOnAnAxisItMayWrite",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexSurfaceMoveDragGuideFollowsWritableAxesTest::RunTest(const FString& Parameters)
{
	using namespace LexSurfaceGestureTestLocal;
	...
	FLexUIPrefabEditorViewportClient::FMoveDragTarget Target = MakeTarget(FVector2D(100.0, 200.0), FVector(0.0, 10.0, 7.0));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexSurfaceReparentRefusalsTest,
	"LGUI.Editor.SurfaceGestures.SurfaceReparentRefusesWhatTheTreeRefuses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexSurfaceReparentRefusalsTest::RunTest(const FString& Parameters)
{
	using namespace LexSurfaceGestureTestLocal;
	FScopedTestWorld TestWorld;

	TStrongObjectPtr<ULexWidget> Root(MakeWidget(TestWorld.World, TEXT("Root"), 800.0f, 600.0f));
	Root->CreateNewLayoutContainer<ULexLayoutContainerCanvasPanel>();
	TStrongObjectPtr<ULexWidget> Panel(MakeWidget(Root.Get(), TEXT("Panel"), 200.0f, 200.0f));
	Panel->TrySetParent(Root.Get(), false);
	Panel->CreateNewLayoutContainer<ULexLayoutContainerCanvasPanel>();
	TStrongObjectPtr<ULexWidget> Grandchild(MakeWidget(Panel.Get(), TEXT("Grandchild"), 50.0f, 50.0f));
	Grandchild->TrySetParent(Panel.Get(), false);
	Grandchild->CreateNewLayoutContainer<ULexLayoutContainerCanvasPanel>();
	TStrongObjectPtr<ULexWidget> Leaf(MakeWidget(Root.Get(), TEXT("Leaf"), 100.0f, 50.0f));
	...
	TStrongObjectPtr<ULexWidget> Full(MakeWidget(TestWorld.World, TEXT("Full"), 200.0f, 200.0f));
	Full->CreateNewLayoutContainer<ULexLayoutContainerSizeBox>();
	TStrongObjectPtr<ULexWidget> Occupant(MakeWidget(Full.Get(), TEXT("Occupant"), 50.0f, 50.0f));
	Occupant->TrySetParent(Full.Get(), false);

	TArray<ULexWidget*> OnlyLeaf = { Leaf.Get() };
	TArray<ULexWidget*> OnlyPanel = { Panel.Get() };
	TArray<ULexWidget*> Nothing;
	TestTrue(TEXT("a widget dropped into a sibling panel with room is a reparent"),
		FLexUIPrefabEditorViewportClient::CanReparentSelectionUnder(OnlyLeaf, Panel.Get()));
	TestFalse(TEXT("a widget cannot be dropped into itself"),
		FLexUIPrefabEditorViewportClient::CanReparentSelectionUnder(OnlyPanel, Panel.Get()));
	TestFalse(TEXT("nor into something it already contains"),
		FLexUIPrefabEditorViewportClient::CanReparentSelectionUnder(OnlyPanel, Grandchild.Get()));
	TestFalse(TEXT("nor into a parent with no room left"),
		FLexUIPrefabEditorViewportClient::CanReparentSelectionUnder(OnlyLeaf, Full.Get()));
	...
	TestFalse(TEXT("a widget dropped where it already lives is not a reparent"),
		FLexUIPrefabEditorViewportClient::CanReparentSelectionUnder(OnlyLeaf, Root.Get()));
	TestFalse(TEXT("and neither an empty selection nor an empty container is one"),
		FLexUIPrefabEditorViewportClient::CanReparentSelectionUnder(Nothing, Panel.Get())
		|| FLexUIPrefabEditorViewportClient::CanReparentSelectionUnder(OnlyLeaf, nullptr));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexSurfacePanelMenuOffersEveryRegisteredPanelTest,
	"LGUI.Editor.SurfaceGestures.PanelMenusOfferEveryRegisteredPanel",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexSurfacePanelMenuOffersEveryRegisteredPanelTest::RunTest(const FString& Parameters)
{
	...
	{
		return Panels.ContainsByPredicate([InClass](const FLexUIControlDescriptor* Descriptor)
		{
	...
	const TArray<UClass*> Registered = {
		...
		ULexLayoutContainerStackBox::StaticClass(),
	...
	TestFalse(TEXT("a control that happens to hang off a panel is not offered as a panel"),
		Contains(ULexLayoutContainerSizeBox::StaticClass()));
	...
	TArray<const FLexUIControlDescriptor*> WithoutGrid;
	FLexUIPrefabEditor::CollectLayoutPanelDescriptors(ULexLayoutContainerGridPanel::StaticClass(), WithoutGrid);
	TestEqual(TEXT("excluding the panel a widget already has drops exactly that one"), WithoutGrid.Num(), Panels.Num() - 1);
	TestFalse(TEXT("and it is the one that is gone"), WithoutGrid.ContainsByPredicate([](const FLexUIControlDescriptor* Descriptor)
	{
		return Descriptor->LayoutContainerClass.Get() == ULexLayoutContainerGridPanel::StaticClass();
	}));
	return true;
}
