...
namespace LexDesignerGestureTestLocal
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexAnchorDragKeepsTheRectTest,
	"LGUI.Editor.DesignerAnchors.MovingAnAnchorLeavesTheRectWhereItIs",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexAnchorDragKeepsTheRectTest::RunTest(const FString& Parameters)
{
	using namespace LexDesignerGestureTestLocal;
	...
	FLexUIPrefabEditorViewportClient::SetAnchorsPreservingRect(Child, FVector2D(0.0f, 0.0f), FVector2D(0.0f, 0.0f));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexAnchorDragToStretchKeepsTheRectTest,
	"LGUI.Editor.DesignerAnchors.StretchingTheAnchorsLeavesTheRectWhereItIs",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexAnchorDragToStretchKeepsTheRectTest::RunTest(const FString& Parameters)
{
	using namespace LexDesignerGestureTestLocal;
	FScopedTestWorld TestWorld;
	...
	FLexUIPrefabEditorViewportClient::SetAnchorsPreservingRect(Child, FVector2D(0.0f, 0.0f), FVector2D(1.0f, 1.0f));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexAnchorSnapTest,
	"LGUI.Editor.DesignerAnchors.NearGridlinesSnapAndTheRestDoNot",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexAnchorSnapTest::RunTest(const FString& Parameters)
{
	TestEqual(TEXT("just past a gridline"), FLexUIPrefabEditorViewportClient::SnapAnchorFraction(0.262, 0.02), 0.25);
	TestEqual(TEXT("just short of the far edge"), FLexUIPrefabEditorViewportClient::SnapAnchorFraction(0.99, 0.02), 1.0);
	...
	TestEqual(TEXT("between two stops"), FLexUIPrefabEditorViewportClient::SnapAnchorFraction(0.4, 0.02), 0.4);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexAnchorAxisPolicyTest,
	"LGUI.Editor.DesignerAnchors.OnlyAxesNothingElseDecidesGetHandles",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexAnchorAxisPolicyTest::RunTest(const FString& Parameters)
{
	using namespace LexDesignerGestureTestLocal;
	FScopedTestWorld TestWorld;

	ULexWidget* Orphan = MakeWidget(TestWorld.World, TEXT("Orphan"), 100.0f, 50.0f);
	bool bHorizontal = true, bVertical = true;
	FLexUIPrefabEditorViewportClient::GetAnchorEditableAxes(Orphan, bHorizontal, bVertical);
	TestFalse(TEXT("a parentless widget has no anchor space at all"), bHorizontal || bVertical);

	ULexWidget* CanvasRoot = MakeWidget(TestWorld.World, TEXT("CanvasRoot"), 800.0f, 600.0f);
	CanvasRoot->CreateNewLayoutContainer<ULexLayoutContainerCanvasPanel>();
	ULexWidget* CanvasChild = MakeWidget(CanvasRoot, TEXT("CanvasChild"), 100.0f, 50.0f);
	CanvasChild->TrySetParent(CanvasRoot, false);
	FLexUIPrefabEditorViewportClient::GetAnchorEditableAxes(CanvasChild, bHorizontal, bVertical);
	...
	ULexWidget* BoxRoot = MakeWidget(TestWorld.World, TEXT("BoxRoot"), 800.0f, 600.0f);
	BoxRoot->CreateNewLayoutContainer<ULexLayoutContainerVerticalBox>();
	ULexWidget* BoxChild = MakeWidget(BoxRoot, TEXT("BoxChild"), 100.0f, 50.0f);
	BoxChild->TrySetParent(BoxRoot, false);
	FLexUIPrefabEditorViewportClient::GetAnchorEditableAxes(BoxChild, bHorizontal, bVertical);
	TestFalse(TEXT("an arranged child gets no anchor handles"), bHorizontal || bVertical);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexMarqueeMeetsRectTest,
	"LGUI.Editor.DesignerMarquee.BoxCatchesOnlyTheRectsItActuallyCrosses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexMarqueeMeetsRectTest::RunTest(const FString& Parameters)
{
	using namespace LexDesignerGestureTestLocal;

	const TArray<FVector2D> Upright = MakeQuad(FVector2D(100, 100), FVector2D(200, 100), FVector2D(200, 160), FVector2D(100, 160));
	TestTrue(TEXT("a box laid over the rect catches it"), FLexUIPrefabEditorViewportClient::DoesMarqueeMeetQuad(FBox2D(FVector2D(80, 90), FVector2D(150, 150)), Upright));
	TestTrue(TEXT("a box entirely inside it counts as crossing it"), FLexUIPrefabEditorViewportClient::DoesMarqueeMeetQuad(FBox2D(FVector2D(120, 120), FVector2D(130, 130)), Upright));
	TestFalse(TEXT("a box beside it does not"), FLexUIPrefabEditorViewportClient::DoesMarqueeMeetQuad(FBox2D(FVector2D(0, 0), FVector2D(60, 60)), Upright));
	...
	TestFalse(TEXT("a corner of the bounding box is not the widget"), FLexUIPrefabEditorViewportClient::DoesMarqueeMeetQuad(FBox2D(FVector2D(52, 52), FVector2D(62, 62)), Diamond));
	TestTrue(TEXT("but its middle is"), FLexUIPrefabEditorViewportClient::DoesMarqueeMeetQuad(FBox2D(FVector2D(95, 95), FVector2D(105, 105)), Diamond));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexMarqueeSelectionModesTest,
	"LGUI.Editor.DesignerMarquee.CtrlAddsAltRemovesNeitherReplaces",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexMarqueeSelectionModesTest::RunTest(const FString& Parameters)
{
	using namespace LexDesignerGestureTestLocal;
	using EMarqueeMode = FLexUIPrefabEditorViewportClient::EMarqueeMode;
	FScopedTestWorld TestWorld;

	ULexWidget* A = MakeWidget(TestWorld.World, TEXT("A"), 10.0f, 10.0f);
	ULexWidget* B = MakeWidget(TestWorld.World, TEXT("B"), 10.0f, 10.0f);
	ULexWidget* C = MakeWidget(TestWorld.World, TEXT("C"), 10.0f, 10.0f);
	const TArray<ULexWidget*> Current{ A, B };
	const TArray<ULexWidget*> Caught{ B, C };

	TSet<ULexWidget*> Result;
	FLexUIPrefabEditorViewportClient::CombineMarqueeSelection(EMarqueeMode::Replace, Current, Caught, Result);
	...
	FLexUIPrefabEditorViewportClient::CombineMarqueeSelection(EMarqueeMode::Add, Current, Caught, Result);
	TestTrue(TEXT("ctrl adds without toggling"), Result.Num() == 3 && Result.Contains(A) && Result.Contains(B) && Result.Contains(C));

	FLexUIPrefabEditorViewportClient::CombineMarqueeSelection(EMarqueeMode::Remove, Current, Caught, Result);
