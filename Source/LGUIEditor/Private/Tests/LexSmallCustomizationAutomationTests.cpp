...
{
	// Defined by LexUIFontData_FreeTypeRenderCustomization.cpp; that customization has no header of
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexSpriteFillOriginMirrorFollowsEverySelectedSpriteTest,
	"LGUI.Editor.SpriteDetails.FillOriginMirrorFollowsEverySelectedSprite",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexSpriteFillOriginMirrorFollowsEverySelectedSpriteTest::RunTest(const FString& Parameters)
{
	using namespace LexSmallCustomizationTestLocal;
	...
	TStrongObjectPtr<ULexSprite> First(NewObject<ULexSprite>(Package));
	TStrongObjectPtr<ULexSprite> Second(NewObject<ULexSprite>(Package));
	...
	const uint8 ExpectedFillOrigin = (uint8)ELexUISpriteFillOriginType_Radial360::Left;
	TArray<ULexSprite*> Sprites = { First.Get(), Second.Get() };
	for (ULexSprite* Sprite : Sprites)
	{
		if (!TestTrue(TEXT("the fixture is a radial-filled sprite with a stale mirror"),
			SetByteProperty(Sprite, TEXT("DrawType"), (uint8)ELexUISpriteDrawType::Filled)
			&& SetByteProperty(Sprite, TEXT("FillMethod"), (uint8)ELexUISpriteFillMethod::Radial360)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexTextureFillOriginMirrorFollowsEverySelectedTextureTest,
	"LGUI.Editor.TextureDetails.FillOriginMirrorFollowsEverySelectedTexture",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexTextureFillOriginMirrorFollowsEverySelectedTextureTest::RunTest(const FString& Parameters)
{
	using namespace LexSmallCustomizationTestLocal;
	UPackage* Package = MakeTestPackage();
	TStrongObjectPtr<ULexTexture> First(NewObject<ULexTexture>(Package));
	TStrongObjectPtr<ULexTexture> Second(NewObject<ULexTexture>(Package));

	const uint8 ExpectedFillOrigin = (uint8)ELexUISpriteFillOriginType_Radial360::Left;
	TArray<ULexTexture*> Textures = { First.Get(), Second.Get() };
	for (ULexTexture* Texture : Textures)
	{
		if (!TestTrue(TEXT("the fixture is a radial-filled texture with a stale mirror"),
			SetByteProperty(Texture, TEXT("DrawType"), (uint8)ELexUISpriteDrawType::Filled)
			&& SetByteProperty(Texture, TEXT("FillMethod"), (uint8)ELexUISpriteFillMethod::Radial360)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexFontRelativePathToggleGoesThroughThePropertyTest,
	"LGUI.Editor.FontDetails.RelativePathToggleGoesThroughTheProperty",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexFontRelativePathToggleGoesThroughThePropertyTest::RunTest(const FString& Parameters)
{
	using namespace LexSmallCustomizationTestLocal;
	using namespace LexUIFontDataCustomization;
	UPackage* Package = MakeTestPackage();
	// Transactional so the property write can record undo the way it does on a real asset.
	TStrongObjectPtr<ULexUIFontData_Bitmap> Font(NewObject<ULexUIFontData_Bitmap>(Package, NAME_None, RF_Transactional));
