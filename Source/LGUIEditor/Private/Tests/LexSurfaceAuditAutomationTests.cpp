...
namespace LexSurfaceAuditTestLocal
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexSurfaceAuditDropContainerTest,
	"LGUI.Editor.SurfaceAudit.DragDropAsksTheContainerThatReceivesNotThePixel",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexSurfaceAuditDropContainerTest::RunTest(const FString& Parameters)
{
	using namespace LexSurfaceAuditTestLocal;
	...
	TStrongObjectPtr<ULexWidget> Label(MakeWidget(Panel.Get(), TEXT("Label"), 50.0f, 20.0f));
	Label->TrySetParent(Panel.Get(), false);
	TStrongObjectPtr<ULexWidget> Mover(MakeWidget(Root.Get(), TEXT("Mover"), 100.0f, 50.0f));
	Mover->TrySetParent(Root.Get(), false);

	TArray<ULexWidget*> Dragged = { Mover.Get() };
	TSet<const ULexWidget*> Locked;
	auto IsLocked = [&Locked](const ULexWidget* InWidget) { return Locked.Contains(InWidget); };

	TestSamePtr(TEXT("a hit inside a panel resolves to the panel that will arrange the drop"),
		FLexUIPrefabEditorViewportClient::ResolveDragDropContainer(Label.Get(), Root.Get(), Dragged, IsLocked), Panel.Get());

	Locked.Add(Panel.Get());
	TestNull(TEXT("and a locked panel stays shut when the cursor is over an unlocked child of it"),
		FLexUIPrefabEditorViewportClient::ResolveDragDropContainer(Label.Get(), Root.Get(), Dragged, IsLocked));
	TestNull(TEXT("as it does when the cursor is on the panel itself"),
		FLexUIPrefabEditorViewportClient::ResolveDragDropContainer(Panel.Get(), Root.Get(), Dragged, IsLocked));
	Locked.Reset();
	...
	TStrongObjectPtr<ULexWidget> BareRoot(MakeWidget(TestWorld.World, TEXT("BareRoot"), 800.0f, 600.0f));
	TStrongObjectPtr<ULexWidget> BareChild(MakeWidget(BareRoot.Get(), TEXT("BareChild"), 100.0f, 100.0f));
	BareChild->TrySetParent(BareRoot.Get(), false);

	TestSamePtr(TEXT("a chain with no container anywhere drops on the prefab root"),
		FLexUIPrefabEditorViewportClient::ResolveDragDropContainer(BareChild.Get(), BareRoot.Get(), Dragged, IsLocked), BareRoot.Get());
	Locked.Add(BareRoot.Get());
	TestNull(TEXT("unless the root is locked too"),
		FLexUIPrefabEditorViewportClient::ResolveDragDropContainer(BareChild.Get(), BareRoot.Get(), Dragged, IsLocked));
	Locked.Reset();

	// The hierarchy's own refusals still have the last word over whatever was resolved.
	TArray<ULexWidget*> DraggingThePanel = { Panel.Get() };
	TestNull(TEXT("and a widget is not dropped into itself however the container was reached"),
		FLexUIPrefabEditorViewportClient::ResolveDragDropContainer(Label.Get(), Root.Get(), DraggingThePanel, IsLocked));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexSurfaceAuditMoveDragAxisLeaderTest,
	"LGUI.Editor.SurfaceAudit.MoveDragSnapsOffAWidgetThatMayHaveTheAxis",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexSurfaceAuditMoveDragAxisLeaderTest::RunTest(const FString& Parameters)
{
	using namespace LexSurfaceAuditTestLocal;
	...
	TArray<FLexUIPrefabEditorViewportClient::FMoveDragTarget> Horizontal;
	...
	FLexUIPrefabEditorViewportClient::ResolveMoveDrag(Horizontal, 8.0f, Results);
	...
	TArray<FLexUIPrefabEditorViewportClient::FMoveDragTarget> Vertical;
	...
	FLexUIPrefabEditorViewportClient::ResolveMoveDrag(Vertical, 8.0f, Results);
	if (!TestEqual(TEXT("one result per target"), Results.Num(), 2))return false;
