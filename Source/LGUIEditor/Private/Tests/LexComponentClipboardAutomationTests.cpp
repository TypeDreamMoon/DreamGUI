...
namespace LexComponentClipboardTestLocal
{
...
// The paste runs after ULexWidget::AddComponent has already registered the new component against the
// target widget, and ULexUIBehaviour::OnUnregister unsubscribes through the cached widget rather than
// through GetWidget() -- so a component whose cache the paste left empty never stops listening.
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexPastedComponentStopsListeningWhenRemovedTest,
	"LGUI.Editor.ComponentClipboard.APastedComponentStopsListeningWhenItIsRemoved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexPastedComponentStopsListeningWhenRemovedTest::RunTest(const FString& Parameters)
{
	using namespace LexComponentClipboardTestLocal;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexPastedComponentDropsTransientStateInsideContainersTest,
	"LGUI.Editor.ComponentClipboard.TransientStateInsideContainersIsNotCarriedAcross",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexPastedComponentDropsTransientStateInsideContainersTest::RunTest(const FString& Parameters)
{
	using namespace LexComponentClipboardTestLocal;
	FScopedTestWorld TestWorld;
	...
	ULexUIBehaviour* Pasted = LexUIWidgetComponentClipboard_PasteOnto(Target.Get(), Dropdown);
	if (!TestNotNull(TEXT("pasting produced a component"), Pasted))return true;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexClipboardRefusesAClassThePickerWouldRefuseTest,
	"LGUI.Editor.ComponentClipboard.AClassThePickerRefusesCannotBePasted",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexClipboardRefusesAClassThePickerWouldRefuseTest::RunTest(const FString& Parameters)
{
	using namespace LexComponentClipboardTestLocal;
	FScopedTestWorld TestWorld;

	TestTrue(TEXT("a spawnable component class is pasteable"),
		LexUIWidgetComponentClipboard_CanPasteClass(UUIButton::StaticClass()));
	...
	TestFalse(TEXT("a class the picker hides is not"),
		LexUIWidgetComponentClipboard_CanPasteClass(ULexScrollBoxInputHandler::StaticClass()));
	TestFalse(TEXT("an abstract base is not"),
		LexUIWidgetComponentClipboard_CanPasteClass(ULexUIBehaviour::StaticClass()));
	TestFalse(TEXT("nothing is not"), LexUIWidgetComponentClipboard_CanPasteClass(nullptr));
	...
	TestNull(TEXT("pasting it produces nothing"), LexUIWidgetComponentClipboard_PasteOnto(Target.Get(), Handler));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexPastedComponentKeepsAuthoredCrossWidgetReferencesTest,
	"LGUI.Editor.ComponentClipboard.AuthoredCrossWidgetReferencesSurviveAPaste",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexPastedComponentKeepsAuthoredCrossWidgetReferencesTest::RunTest(const FString& Parameters)
{
	using namespace LexComponentClipboardTestLocal;
	FScopedTestWorld TestWorld;
	...
	TStrongObjectPtr<ULexWidget> Source = MakeWidget(TestWorld.World, TEXT("Source"));
	TStrongObjectPtr<ULexWidget> Target = MakeWidget(TestWorld.World, TEXT("Target"));
	TStrongObjectPtr<ULexWidget> Elsewhere = MakeWidget(TestWorld.World, TEXT("Elsewhere"));

	UUIButton* Button = Source->AddComponent<UUIButton>();
	UUIButton* ForeignSelectable = Elsewhere->AddComponent<UUIButton>();
	ULexVisual* ForeignVisual = Elsewhere->CreateNewVisual<ULexVisualEmpty>();
	...
	UUISelectable* Pasted = Cast<UUISelectable>(LexUIWidgetComponentClipboard_PasteOnto(Target.Get(), Button));
