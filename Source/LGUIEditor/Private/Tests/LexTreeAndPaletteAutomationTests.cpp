...
#include "PrefabEditor/LexWidgetEditorHierarchyViewItem.h"
#include "PrefabEditor/SLexUIPrefabPalette.h"
...
//
// Everything in the hierarchy is a ULexWidget -- what makes one a text block and another a vertical
...
namespace LexTreeAndPaletteTestLocal
{
	...
	LexUIPalette::FItemPtr MakeGroup(const TCHAR* DisplayName)
	{
		LexUIPalette::FItemPtr Group = MakeShared<LexUIPalette::FPaletteItem>();
		Group->Kind = LexUIPalette::EItemKind::Category;
		Group->DisplayName = DisplayName;
		return Group;
	}

	LexUIPalette::FItemPtr AddEntry(const LexUIPalette::FItemPtr& Group, LexUIPalette::EItemKind Kind, const TCHAR* DisplayName)
	{
		LexUIPalette::FItemPtr Item = MakeShared<LexUIPalette::FPaletteItem>();
		Item->Kind = Kind;
	...
	bool MatchesText(const LexUIPalette::FPaletteItem& Item)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexHierarchyTypeIsSearchableTest,
	"LGUI.Editor.HierarchySearch.WidgetTypeIsSearchableAndLabelled",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexHierarchyTypeIsSearchableTest::RunTest(const FString& Parameters)
{
	using namespace LexTreeAndPaletteTestLocal;
	...
	TStrongObjectPtr<ULexWidget> Plain(MakeWidget(TestWorld.World, TEXT("Plain")));
	TArray<FString> PlainTerms;
	LexWidgetHierarchyType::CollectSearchTerms(Plain.Get(), PlainTerms);
	...
	TestTrue(TEXT("a plain widget prints no type label"), LexWidgetHierarchyType::GetTypeLabel(Plain.Get()).IsEmpty());

	// The visual is what "Text"/"Image" means to whoever is searching.
	TStrongObjectPtr<ULexWidget> Pictured(MakeWidget(TestWorld.World, TEXT("Banner")));
	Pictured->CreateNewVisual<ULexImage>();
	TArray<FString> PicturedTerms;
	LexWidgetHierarchyType::CollectSearchTerms(Pictured.Get(), PicturedTerms);
	TestTrue(TEXT("the visual's class is searchable"), PicturedTerms.Contains(TEXT("LexImage")));
	TestTrue(TEXT("a widget with a visual is labelled by it"), LexWidgetHierarchyType::GetTypeLabel(Pictured.Get()).Contains(TEXT("Image")));

	// The layout container is what "vertical box" means, and it is the label when nothing is drawn.
	TStrongObjectPtr<ULexWidget> Column(MakeWidget(TestWorld.World, TEXT("Column")));
	Column->CreateNewLayoutContainer<ULexLayoutContainerVerticalBox>();
	TArray<FString> ColumnTerms;
	LexWidgetHierarchyType::CollectSearchTerms(Column.Get(), ColumnTerms);
	TestTrue(TEXT("the layout container's class is searchable"), ColumnTerms.Contains(TEXT("LexLayoutContainerVerticalBox")));
	TestTrue(TEXT("a layout-only widget is labelled by its container"), LexWidgetHierarchyType::GetTypeLabel(Column.Get()).Contains(TEXT("Vertical")));

	// Behaviours carry the interactive types, which is most of what a control IS.
	TStrongObjectPtr<ULexWidget> Behaving(MakeWidget(TestWorld.World, TEXT("Slot")));
	Behaving->AddComponent<ULexContentWidget>();
	TArray<FString> BehavingTerms;
	LexWidgetHierarchyType::CollectSearchTerms(Behaving.Get(), BehavingTerms);
	TestTrue(TEXT("a behaviour's class is searchable"), BehavingTerms.Contains(TEXT("LexContentWidget")));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexHierarchyDropLockOwnerTest,
	"LGUI.Editor.HierarchyDrop.SiblingDropAsksTheParentsLock",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexHierarchyDropLockOwnerTest::RunTest(const FString& Parameters)
{
	using namespace LexTreeAndPaletteTestLocal;
	using namespace LexWidgetHierarchyDrop;
	FScopedTestWorld TestWorld;
	...
	ULexWidget* Child = MakeWidget(Root.Get(), TEXT("Child"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexPaletteFavoritesTest,
	"LGUI.Editor.Palette.FavoritesLeadTheTreeAndKeepTheirOwnKeys",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexPaletteFavoritesTest::RunTest(const FString& Parameters)
{
	using namespace LexTreeAndPaletteTestLocal;
	using namespace LexUIPalette;
	...
	Labelled.NativeDescriptor = MakeShared<FLexUIControlDescriptor>();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexPaletteGroupExpansionTest,
	"LGUI.Editor.Palette.CollapsedGroupsSurviveARebuild",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexPaletteGroupExpansionTest::RunTest(const FString& Parameters)
{
	using namespace LexUIPalette;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLexControlRegistryRefusalTest,
	"LGUI.Editor.ControlRegistry.ARefusedRegistrationSaysSo",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLexControlRegistryRefusalTest::RunTest(const FString& Parameters)
{
	...
	FLexUIControlRegistry& Registry = FLexUIControlRegistry::Get();
	FLexUIControlDescriptor Descriptor;
	Descriptor.Name = TEXT("LGUIAutomationTest.Control");
	Descriptor.DisplayName = FText::FromString(TEXT("Automation Test Control"));
	Descriptor.Category = TEXT("Automation");
	Descriptor.VisualClass = ULexImage::StaticClass();
	...
	FLexUIControlDescriptor Nameless = Descriptor;
