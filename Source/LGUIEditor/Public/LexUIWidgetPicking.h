 ...
 *
 * ULexUIManagerWorldSubsystem::RaycastHitUI answers "what did the player click", so it starts from
 * `if (auto Visual = Widget->GetVisual())` -- no mesh, no hit. Every UMG-family layout panel is
 * registered with a layout container and no visual (FLexUIControlRegistry::MakePanel), so under that
...
namespace LexUIWidgetPicking
{
	...
	LGUIEDITOR_API bool LineTraceWidgetRect(const ULexWidget* InWidget, const FVector& InLineStart, const FVector& InLineEnd, FVector& OutWorldLocation);

	/** Every crossed rect among InWidgets, front-to-back: nearest canvas first, then deepest child. */
	LGUIEDITOR_API void RaycastWidgetRects(const UWorld* InWorld, TConstArrayView<ULexWidget*> InWidgets, const FVector& InLineStart, const FVector& InLineEnd, TArray<FLexUIWidgetPickHit>& OutHits);
	 ...
	 */
	LGUIEDITOR_API ULexWidget* PickTopmostWidget(const UWorld* InWorld, TConstArrayView<ULexWidget*> InWidgets, const FVector& InLineStart, const FVector& InLineEnd, int32& InOutCycleIndex);
	 ...
	 */
	LGUIEDITOR_API ULexWidget* ResolveDropContainer(ULexWidget* InHitWidget);

	/** Every widget under every root canvas of InWorld, the canvas roots included. */
	LGUIEDITOR_API void CollectPickableWidgets(const UWorld* InWorld, TArray<ULexWidget*>& OutWidgets);
}
