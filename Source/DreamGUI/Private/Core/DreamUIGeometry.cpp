...
#pragma region DreamText
#include "Core/Components/DreamText.h"
void UIGeometry_AlignUITextLineVertex(EDreamUITextParagraphHorizontalAlign pivotHAlign, float lineWidth, int lineUIGeoVertStart
	, TArray<FDreamUIOriginVertexData>& vertices, FDreamUITextLineProperty& lineProperty
	, int lineEmojiStartIndex, TArray<FDreamUIText_Emoji>& emojiArray
)
{
	float xOffset = 0;
	switch (pivotHAlign)
...
void UIGeometry_AlignUITextLineVertexForRichText(EDreamUITextParagraphHorizontalAlign pivotHAlign, float lineWidth, float lineHeight, float fontSize, int lineUIGeoVertStart
	, TArray<FDreamUIOriginVertexData>& vertices
	, int lineImageStartIndex, TArray<FDreamUIText_RichTextImageTag>& imageArray
	, int lineEmojiStartIndex, TArray<FDreamUIText_Emoji>& emojiArray
)
{
	float xOffset = 0;
...
void FDreamUIGeometry::UpdateUIText(const FString& Content
	...
	, UDreamCanvas* renderCanvas, UDreamText* DreamText
	...
	, UDreamUIFontData_BaseObject* font, bool bRichText, int32 richTextFilterFlags)
{
	...
	bool pixelPerfect = DreamText->GetShouldAffectByPixelSnapping() && DreamText->GetWidget()->GetPixelSnappingInHierarchy();
	float rootCanvasScale = renderCanvas->GetRootCanvas()->GetCanvasScale();
	float dynamicPixelsPerUnit = DreamText->GetDynamicPixelsPerUnit() * rootCanvasScale;
	...
	auto richTextImageData = DreamText->GetRichTextImageData();
	auto GetRichTextImageCharData = [&](FDreamUICharData& overrideCharData, float inFontSize, FName imageTag)
	{
	...
	auto GetEmojiCharData = [&](FDreamUICharData& overrideCharData, float inFontSize, uint32 emojiCode)
	{
	...
	font->PrepareForPushCharData(DreamText);
	bool useKerning = bUseKerning && font->HasKerning();

	bool bUseBold = fontStyle == EDreamUITextFontStyle::Bold || fontStyle == EDreamUITextFontStyle::BoldAndItalic;
	bool bUseItalic = fontStyle == EDreamUITextFontStyle::Italic || fontStyle == EDreamUITextFontStyle::BoldAndItalic;
	...
	const float lineHeightScale = FMath::Max(0.0f, DreamText->GetLineHeightPercentage());
	...
	const float authoredWrapAt = DreamText->GetWrapTextAt();
	...
	auto GetCharGeo = [&](uint32 prevCharCode, const FDreamUIText_TextProcessingElement& charElement, float inFontSize, bool inBold)
	{
	...
	auto GetCharGeoXAdv = [&](uint32 prevCharCode, const FDreamUIText_TextProcessingElement& charElement, const FRichTextParseResult& richTextResult)
	{
	...
	{
		//pre parse rich text
		auto richTextCustomStyleData = DreamText->GetRichTextCustomStyleData();
		...
		for (int charIndex = 0; charIndex < contentLength; charIndex++)
		{
			...
			while (richTextParser.Parse(Content, contentLength, charIndex, richTextParseResult))
			{
				...
				{
					TextProcessingArray.Add(FDreamUIText_TextProcessingElement{' ', charIndex, 1});
			...
			if (richTextParseResult.CustomTagMode == DreamUIRichTextParser::ECustomTagMode::End)
			{
				...
				richTextParseResult.CustomTag = NAME_None;
				richTextParseResult.CustomTagMode = DreamUIRichTextParser::ECustomTagMode::None;
			}
			...
			TextProcessingArray.Add(FDreamUIText_CodePoint::ReadCodePoint(Content, contentLength, charIndex));
		}
	}
	else
	{
	...
	for (int charIndex = 0; charIndex < contentLength; charIndex++)
	{
		...
		{
			if (overflowType == EDreamUITextOverflowType::VerticalOverflow//char is space and DreamText can have overflow line, then we need to calculate if the following words can fit the rest space, if not means new line
				)
			{
		...
		if (bRichText)
		{
			...
			{
			case DreamUIRichTextParser::ECustomTagMode::Start:
			{
			...
			break;
			case DreamUIRichTextParser::ECustomTagMode::End:
			{
				int foundIndex = cacheRichTextCustomTagArray.IndexOfByPredicate([richTextParseResult](const FDreamUIText_RichTextCustomTag& A) {
		...
		//overflow
		{
			...
			case EDreamUITextOverflowType::Ellipsis:
			{
				...
				if (currentLineOffset.X + nextCharXAdv > width)//horizontal cannot fit next char
				{
					...
					if (overflowType == EDreamUITextOverflowType::Ellipsis)
					{
						//move back and replace chars by ...
						uint32 charCodeOfDots = 0x2026;//'…'
						auto charElementOfDots = FDreamUIText_TextProcessingElement{charCodeOfDots, charIndex, 1, EDreamUIText_CodeType::Text};
	...
	FDreamUIGeometry::OffsetVertices(originVertices, originVertices.Num(), xOffset, yOffset);

	//snap pixel
	if (pixelPerfect)
	{
		AdjustPixelPerfectPos_For_UIText(originVertices, cacheCharPropertyArray, renderCanvas, DreamText);
	}
}
