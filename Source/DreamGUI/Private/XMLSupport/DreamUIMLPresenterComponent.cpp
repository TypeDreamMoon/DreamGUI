...
UDreamUIMLPresenterComponent::UDreamUIMLPresenterComponent()
{
...
void UDreamUIMLPresenterComponent::BeginPlay()
{
	Super::BeginPlay();
}

void UDreamUIMLPresenterComponent::OnRegister()
{
	Super::OnRegister();
#if WITH_EDITOR
...
void UDreamUIMLPresenterComponent::OnUnregister()
{
	Super::OnUnregister();
#if WITH_EDITOR
...
void UDreamUIMLPresenterComponent::LoadWidget()
{
	...
	{
		UE_LOG(DreamGUI, Warning, TEXT("[%s].%d - Skip LoadWidget for %s (temp object)"), ANSI_TO_TCHAR(__FUNCTION__), __LINE__, *this->GetName());
		return;
	}
#endif
	...
	auto Behaviour = UDreamUIMLBehaviour::CreateByClass(XAMLScriptClass, World, nullptr, nullptr, false, nullptr);
	if (!Behaviour)
	{
		UE_LOG(DreamGUI, Error, TEXT("[%s].%d - Failed to load Widget with class: %s"), ANSI_TO_TCHAR(__FUNCTION__), __LINE__, *XAMLScriptClass->GetPathName());
		return;
	}
	...
	RootCanvas = LoadedWidget->AddComponentByTemplate<UDreamCanvas>(CanvasTemplate);
...
void UDreamUIMLPresenterComponent::RegisterEditorFocusCheck()
{
...
void UDreamUIMLPresenterComponent::UnregisterEditorFocusCheck()
{
	if (EditorFocusHandle.IsValid())
	{
...
void UDreamUIMLPresenterComponent::CheckAndReloadWidget()
{
	...
	{
		UE_LOG(DreamGUI, Log, TEXT("[%s].%d - Source file changed, reloading: %s"), ANSI_TO_TCHAR(__FUNCTION__), __LINE__, *FilePath);
		CachedFileMD5 = CurrentMD5;
		ReloadWidget();
	}
}
...
void UDreamUIMLPresenterComponent::SetScriptClass(TSubclassOf<UDreamUIMLBehaviour> Value)
{
...
FString UDreamUIMLPresenterComponent::GetXAMLFilePath() const
{
	if (XAMLScriptClass)
	{
		FString RelativePath;
		UDreamUIMLResource* Resource = nullptr;
		GetDefault<UDreamUIMLBehaviour>(XAMLScriptClass)->GetUIMLData(RelativePath, Resource);
		if (!RelativePath.IsEmpty())
		{
			return UDreamUIMLBehaviour::ResolveUIMLPath(RelativePath);
		}
	}
	return FString();
}
