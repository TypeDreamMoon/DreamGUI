...
namespace DreamUIPrefabSystem
{
	...
	bool WidgetSerializerBase::CollectObjectToSerialize(UObject* Object, FGuid& OutGuid)
	{
		...
		if (Object->IsA<UDreamWidget>())return false;//skip Widget, because Widget is collected in WillSerializeWidgetArray
	...
	TMap<UObject*, TArray<uint8>> WidgetSerializerBase::SaveOverrideParameterToData(TArray<FDreamUIPrefabOverrideParameterData> InData)
	{
		...
		for (auto& DataItem : InData)
		{
			...
			FDreamUIImmediateOverrideParameterObjectWriter Writer(DataItem.Object.Get(), ObjectOverrideData, *this, DataItem.MemberPropertyNames);
			MapObjectToOverrideDatas.Add(DataItem.Object.Get(), ObjectOverrideData);
		}
		return MapObjectToOverrideDatas;
	}

	void WidgetSerializerBase::RestoreOverrideParameterFromData(TMap<UObject*, TArray<uint8>>& InData, TArray<FDreamUIPrefabOverrideParameterData> InNameSetData)
	{
		this->bIsEditorOrRuntime = true;
		for (auto& KeyValue : InData)
		{
			...
			{
				auto Index = InNameSetData.IndexOfByPredicate([&](const FDreamUIPrefabOverrideParameterData& Item) {
					return Item.Object.Get() == KeyValue.Key;
					});
				if (Index != INDEX_NONE)
				{
					FDreamUIImmediateOverrideParameterObjectReader Reader(KeyValue.Key, KeyValue.Value, *this, InNameSetData[Index].MemberPropertyNames);
				}
			}
		}
	}
