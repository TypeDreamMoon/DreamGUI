...
#include "PrefabSystem/DreamUIPrefabScene.h"
...
FDreamUIPrefabScene::FDreamUIPrefabScene(FDreamUIPrefabScene::ConstructionValues CVS)
	...
	, bForceAllUsedMipsResident(CVS.bForceMipsResident)
{
	...
	FCoreDelegates::OnEnginePreExit.AddRaw(this, &FDreamUIPrefabScene::Uninitialize);
}

FDreamUIPrefabScene::~FDreamUIPrefabScene()
{
	FCoreDelegates::OnEnginePreExit.RemoveAll(this);
	Uninitialize();
}

void FDreamUIPrefabScene::Uninitialize()
{
	if (!PreviewWorld)
	{
		return;
...
void FDreamUIPrefabScene::AddComponent(UActorComponent* Component,const FTransform& LocalToWorld, bool bAttachToRoot /*= false*/)
{
...
void FDreamUIPrefabScene::RemoveComponent(UActorComponent* Component)
{
	Component->UnregisterComponent();
...
void FDreamUIPrefabScene::AddReferencedObjects( FReferenceCollector& Collector )
{
	Collector.AddReferencedObjects( Components );
	Collector.AddReferencedObject( PreviewWorld );
}

FString FDreamUIPrefabScene::GetReferencerName() const
{
	return TEXT("FDreamUIPrefabScene");
}

void FDreamUIPrefabScene::UpdateCaptureContents()
{
...
void FDreamUIPrefabScene::ClearLineBatcher()
{
	if (LineBatcher != NULL)
	{
		LineBatcher->Flush();
...
FRotator FDreamUIPrefabScene::GetLightDirection()
{
...
void FDreamUIPrefabScene::SetLightDirection(const FRotator& InLightDir)
{
#if WITH_EDITOR
...
void FDreamUIPrefabScene::SetLightBrightness(float LightBrightness)
{
#if WITH_EDITOR
	DirectionalLight->PreEditChange(NULL);
#endif // WITH_EDITOR
...
void FDreamUIPrefabScene::SetLightColor(const FColor& LightColor)
{
#if WITH_EDITOR
	DirectionalLight->PreEditChange(NULL);
#endif // WITH_EDITOR
...
void FDreamUIPrefabScene::SetSkyBrightness(float SkyBrightness)
{
	SkyLight->SetIntensity(SkyBrightness);
}

void FDreamUIPrefabScene::SetSkyCubemap(UTextureCube* Cubemap)
{
	SkyLight->SetCubemap(Cubemap);
}

void FDreamUIPrefabScene::LoadSettings(const TCHAR* Section)
{
	FRotator LightDir;
...
void FDreamUIPrefabScene::SaveSettings(const TCHAR* Section)
{
	GConfig->SetRotator( Section, TEXT("LightDir"), GetLightDirection(), GEditorPerProjectIni );
}

FLinearColor FDreamUIPrefabScene::GetBackgroundColor() const
{
