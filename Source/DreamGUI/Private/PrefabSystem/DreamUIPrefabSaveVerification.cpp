...
namespace SaveVerificationLocal
{
	...
	void CollectHierarchy(UDreamWidget* Widget, const FString& Path, const bool bIsRoot, TArray<FNodeRecord>& Out)
	{
		if (!IsValid(Widget))
		{
			return;
		...
		if (UDreamLayoutContainer* Layout = Widget->GetLayoutContainer(); IsValid(Layout))
		{
		...
		if (!bIsRoot)
		{
			if (UDreamPanelSlot* Slot = Widget->GetPanelSlot(); IsValid(Slot))
			{
				Out.Add({ Path, TEXT("slot"), Slot });
			}
		}
		const TArray<UDreamUIBehaviour*> Components = Widget->GetComponents(UDreamUIBehaviour::StaticClass());
		...
		, TArray<FString>& OutPropertyDifferences)
	{
		...
		if (const UDreamWidget* ReloadedWidget = Cast<UDreamWidget>(Reloaded.Object))
		{
			const UDreamPanelSlot* ReloadedSlot = ReloadedWidget->GetPanelSlot();
...
FDreamUIPrefabSaveVerificationResult VerifyPrefabSaveRoundTrip(UDreamUIPrefab* InPrefab, UDreamWidget* InSourceRoot)
{
	using namespace SaveVerificationLocal;

	FDreamUIPrefabSaveVerificationResult Result;
	...
	UDreamWidget* ReloadedRoot = nullptr;
	{
...
void FDreamUIPrefabEditorPayloadSnapshot::Capture(const UDreamUIPrefab* InPrefab)
{
...
void FDreamUIPrefabEditorPayloadSnapshot::Restore(UDreamUIPrefab* InPrefab) const
{
