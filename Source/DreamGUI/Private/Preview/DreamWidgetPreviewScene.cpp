...
FDreamWidgetPreviewScene::FDreamWidgetPreviewScene(FDreamWidgetPreviewScene::ConstructionValues CVS)
	...
	, bForceAllUsedMipsResident(CVS.bForceMipsResident)
{
	...
	FCoreDelegates::OnEnginePreExit.AddRaw(this, &FDreamWidgetPreviewScene::Uninitialize);
}

FDreamWidgetPreviewScene::~FDreamWidgetPreviewScene()
{
	FCoreDelegates::OnEnginePreExit.RemoveAll(this);
	Uninitialize();
}

void FDreamWidgetPreviewScene::Uninitialize()
{
	if (!PreviewWorld)
	{
		return;
...
void FDreamWidgetPreviewScene::AddComponent(UActorComponent* Component,const FTransform& LocalToWorld, bool bAttachToRoot /*= false*/)
{
...
void FDreamWidgetPreviewScene::RemoveComponent(UActorComponent* Component)
{
	Component->UnregisterComponent();
...
void FDreamWidgetPreviewScene::AddReferencedObjects( FReferenceCollector& Collector )
{
	Collector.AddReferencedObjects( Components );
	Collector.AddReferencedObject( PreviewWorld );
}

FString FDreamWidgetPreviewScene::GetReferencerName() const
{
	return TEXT("FDreamWidgetPreviewScene");
}
...
void FDreamWidgetPreviewScene::ClearLineBatcher()
{
	if (LineBatcher != NULL)
	{
		LineBatcher->Flush();
...
FRotator FDreamWidgetPreviewScene::GetLightDirection()
{
...
void FDreamWidgetPreviewScene::SetLightDirection(const FRotator& InLightDir)
{
#if WITH_EDITOR
...
void FDreamWidgetPreviewScene::SetLightBrightness(float LightBrightness)
{
#if WITH_EDITOR
	DirectionalLight->PreEditChange(NULL);
#endif // WITH_EDITOR
...
void FDreamWidgetPreviewScene::SetLightColor(const FColor& LightColor)
{
#if WITH_EDITOR
	DirectionalLight->PreEditChange(NULL);
#endif // WITH_EDITOR
...
void FDreamWidgetPreviewScene::SetSkyBrightness(float SkyBrightness)
{
	SkyLight->SetIntensity(SkyBrightness);
}

void FDreamWidgetPreviewScene::SetSkyCubemap(UTextureCube* Cubemap)
{
	SkyLight->SetCubemap(Cubemap);
}

void FDreamWidgetPreviewScene::LoadSettings(const TCHAR* Section)
{
	FRotator LightDir;
...
void FDreamWidgetPreviewScene::SaveSettings(const TCHAR* Section)
{
	GConfig->SetRotator( Section, TEXT("LightDir"), GetLightDirection(), GEditorPerProjectIni );
}

FLinearColor FDreamWidgetPreviewScene::GetBackgroundColor() const
{
