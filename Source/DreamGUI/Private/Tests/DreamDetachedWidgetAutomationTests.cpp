 ...
 * UDreamWidget IS the drawn thing -- so the state has to be built rather than inherited, and these
...
namespace DreamDetachedWidgetTestLocal
{
	...
	UDreamWidget* MakeParkedWidget(UWorld* World, const TCHAR* Name, bool bWithOwnCanvas = false)
	{
		...
		if (bWithOwnCanvas)
		{
			Widget->AddComponent<UDreamCanvas>();
		}
		Widget->OnRegister();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamParkedWidgetIsInertTest,
	"DreamGUI.Widget.Parked.RegisteredButInactiveIsInert",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamParkedWidgetIsInertTest::RunTest(const FString& Parameters)
{
	using namespace DreamDetachedWidgetTestLocal;
	...
	// Inertness comes from the active flag: it is what UDreamUIBehaviour::BeginPlay gates OnEnable and
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamParkedWidgetSubObjectsTest,
	"DreamGUI.Widget.Parked.SubObjectsCanStillBeBuilt",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamParkedWidgetSubObjectsTest::RunTest(const FString& Parameters)
{
	using namespace DreamDetachedWidgetTestLocal;
	FScopedGameWorld TestWorld;
	UDreamWidget* Parked = MakeParkedWidget(TestWorld.World, TEXT("Parked"));
	...
	UDreamLayoutContainer* Container = Parked->CreateNewLayoutContainer<UDreamLayoutContainerVerticalBox>();
	...
	UDreamVisual* Visual = Parked->CreateNewVisual<UDreamVisualEmpty>();
	...
	UDreamWidget* Child = MakeParkedWidget(TestWorld.World, TEXT("Child"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamParkedWidgetOwnCanvasTest,
	"DreamGUI.Widget.Parked.OwnCanvasDoesNotRenderWhileParked",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamParkedWidgetOwnCanvasTest::RunTest(const FString& Parameters)
{
	using namespace DreamDetachedWidgetTestLocal;
	FScopedGameWorld TestWorld;

	// The trap: OnHierarchyAttachmentChanged takes the widget's OWN canvas as its render canvas when
	// it has one, so a page prefab -- whose root almost always carries a UDreamCanvas -- becomes a
	...
	UDreamWidget* Parked = MakeParkedWidget(TestWorld.World, TEXT("ParkedPage"), /*bWithOwnCanvas*/true);
	UDreamCanvas* OwnCanvas = Parked->GetComponent<UDreamCanvas>();
	...
	// and DreamCanvas gates UpdateVisual on GetRenderVisibleInHierarchy -- which folds in the active
	...
	OwnCanvas->SetRenderMode(EDreamRenderMode::ScreenSpaceOverlay);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamWidgetRegisterIsIdempotentTest,
	"DreamGUI.Widget.Parked.RegisterTwiceDoesNotLeakAPropertySlot",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamWidgetRegisterIsIdempotentTest::RunTest(const FString& Parameters)
{
	using namespace DreamDetachedWidgetTestLocal;
	FScopedGameWorld TestWorld;
	...
	// early-outs -- but UDreamCanvas::RegisterVisual calls RegisterBuffer() unconditionally and
	...
	UDreamWidget* Root = MakeParkedWidget(TestWorld.World, TEXT("Root"), /*bWithOwnCanvas*/true);
	Root->SetWidgetActive(true);

	UDreamWidget* Child = MakeParkedWidget(TestWorld.World, TEXT("Child"));
	UDreamVisual* Visual = Child->CreateNewVisual<UDreamVisualEmpty>();
