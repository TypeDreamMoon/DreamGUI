...
namespace DreamParkedSweepTestLocal
{
	...
	struct FScopedParkedLifetime
	{
		...
		{
			GetMutableDefault<UDreamUISettings>()->ParkedWidgetLifetimeSeconds = Previous;
		}
	};
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamParkedSweepIsOffByDefaultTest,
	"DreamGUI.Widget.Pending.SweepIsOffUnlessAskedFor",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamParkedSweepIsOffByDefaultTest::RunTest(const FString& Parameters)
{
	using namespace DreamParkedSweepTestLocal;
	...
	TestEqual(TEXT("The setting ships off"), GetDefault<UDreamUISettings>()->ParkedWidgetLifetimeSeconds, 0.0f);

	FScopedParkedLifetime Lifetime(0.0f);
	UDreamWidget* Waiting = UDreamUIBPLibrary::ConstructWidget(TestWorld.World, TEXT("Waiting"), nullptr);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamParkedSweepDestroysTest,
	"DreamGUI.Widget.Pending.ExpiredWidgetsAreDestroyedNotDropped",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamParkedSweepDestroysTest::RunTest(const FString& Parameters)
{
	using namespace DreamParkedSweepTestLocal;
	FScopedGameWorld TestWorld;
	UDreamUIManagerWorldSubsystem* Manager = UDreamUIManagerWorldSubsystem::GetInstance(TestWorld.World);
	...
	UDreamWidget* Forgotten = UDreamUIBPLibrary::ConstructWidget(TestWorld.World, TEXT("Forgotten"), nullptr);
	...
	// for GC, and UDreamWidget::BeginDestroy answers that with an error and an on-screen banner from a
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamParkedSweepSparesAddedTest,
	"DreamGUI.Widget.Pending.AddedWidgetsAreNeverSwept",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamParkedSweepSparesAddedTest::RunTest(const FString& Parameters)
{
	using namespace DreamParkedSweepTestLocal;
	FScopedGameWorld TestWorld;
	UDreamUIManagerWorldSubsystem* Manager = UDreamUIManagerWorldSubsystem::GetInstance(TestWorld.World);
	...
	UDreamWidget* Added = UDreamUIBPLibrary::ConstructWidget(TestWorld.World, TEXT("Added"), nullptr);
	UDreamWidget* Forgotten = UDreamUIBPLibrary::ConstructWidget(TestWorld.World, TEXT("Forgotten"), nullptr);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamParkedSweepTakesSubtreeTest,
	"DreamGUI.Widget.Pending.SweepingAForgottenRootTakesItsSubtree",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamParkedSweepTakesSubtreeTest::RunTest(const FString& Parameters)
{
	using namespace DreamParkedSweepTestLocal;
	FScopedGameWorld TestWorld;
	UDreamUIManagerWorldSubsystem* Manager = UDreamUIManagerWorldSubsystem::GetInstance(TestWorld.World);
	...
	UDreamWidget* Grandchild = UDreamUIBPLibrary::ConstructWidget(TestWorld.World, TEXT("Grandchild"), nullptr);
