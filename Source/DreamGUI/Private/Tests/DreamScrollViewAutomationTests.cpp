...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamAnchoredScrollViewTest,
	"DreamGUI.Interaction.ScrollView.AnchoredProgress",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamAnchoredScrollViewTest::RunTest(const FString& Parameters)
{
	UDreamWidget* Viewport = NewObject<UDreamWidget>(GetTransientPackage());
	Viewport->SetWidth(320.0f);
	Viewport->SetHeight(200.0f);
	Viewport->SetPivot(FVector2D(0.5f));

	UDreamWidget* Content = NewObject<UDreamWidget>(Viewport);
	...
	UDreamPointerEventData* ScrollEvent = NewObject<UDreamPointerEventData>();
