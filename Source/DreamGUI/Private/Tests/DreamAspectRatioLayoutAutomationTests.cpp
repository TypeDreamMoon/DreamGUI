...
/*
 * UDreamLayoutSelfAspectRatio used to ask one all-or-nothing question - "does the parent panel own this
...
namespace DreamAspectRatioLayoutTestLocal
{
	...
	struct FPanelWithAspectChildFixture
	{
		...
		UDreamWidget* Aspect = nullptr;
		UDreamLayoutSelfAspectRatio* AspectSelf = nullptr;

		bool Build(UWorld* World, EDreamLayoutAspectRatioType Type)
		{
			Root = NewObject<UDreamWidget>(World);
			Plain = NewObject<UDreamWidget>(Root);
			Aspect = NewObject<UDreamWidget>(Root);
			...
			if (!Root->CreateNewLayoutContainer<UDreamLayoutContainerHorizontalBox>())
			{
				return false;
			}
			AspectSelf = Aspect->CreateNewLayoutSelf<UDreamLayoutSelfAspectRatio>();
			if (!AspectSelf)
			{
				return false;
			}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamAspectRatioUnderPanelConvergesTest,
	"DreamGUI.Layout.AspectRatio.FitInParentUnderPanelConverges",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamAspectRatioUnderPanelConvergesTest::RunTest(const FString& Parameters)
{
	...
	if (!TestNotNull(TEXT("DreamUI manager subsystem exists"), Manager)
		|| !Fixture.Build(TestWorld.World, EDreamLayoutAspectRatioType::FitInParent))
	{
		return false;
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamAspectRatioEnvelopeUnderPanelConvergesTest,
	"DreamGUI.Layout.AspectRatio.EnvelopeParentUnderPanelConverges",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamAspectRatioEnvelopeUnderPanelConvergesTest::RunTest(const FString& Parameters)
{
	using namespace DreamAspectRatioLayoutTestLocal;
	FScopedTestWorld TestWorld;
	UDreamUIManagerWorldSubsystem* Manager = UDreamUIManagerWorldSubsystem::GetInstance(TestWorld.World);
	FPanelWithAspectChildFixture Fixture;
	if (!TestNotNull(TEXT("DreamUI manager subsystem exists"), Manager)
		|| !Fixture.Build(TestWorld.World, EDreamLayoutAspectRatioType::EnvelopeParent))
	{
		return false;
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamAspectRatioMeasurementDoesNotWriteTest,
	"DreamGUI.Layout.AspectRatio.MeasurementDoesNotWriteTheWidget",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamAspectRatioMeasurementDoesNotWriteTest::RunTest(const FString& Parameters)
{
	using namespace DreamAspectRatioLayoutTestLocal;
	FScopedTestWorld TestWorld;
	...
	UDreamApplyCountingAspectRatio* AspectSelf = Child->CreateNewLayoutSelf<UDreamApplyCountingAspectRatio>();
	if (!TestNotNull(TEXT("AspectRatio created"), AspectSelf))
	{
		return false;
	}
	AspectSelf->SetAspectRatio(1.0f);
	AspectSelf->SetAspectRatioType(EDreamLayoutAspectRatioType::FitInParent);
	Root->OnRegister();
	Child->OnRegister();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamAspectRatioWithoutParentLayoutStillOwnsGeometryTest,
	"DreamGUI.Layout.AspectRatio.WithoutParentLayoutStillOwnsGeometry",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamAspectRatioWithoutParentLayoutStillOwnsGeometryTest::RunTest(const FString& Parameters)
{
	using namespace DreamAspectRatioLayoutTestLocal;
	FScopedTestWorld TestWorld;
	UDreamUIManagerWorldSubsystem* Manager = UDreamUIManagerWorldSubsystem::GetInstance(TestWorld.World);
	...
	UDreamLayoutSelfAspectRatio* AspectSelf = Child->CreateNewLayoutSelf<UDreamLayoutSelfAspectRatio>();
