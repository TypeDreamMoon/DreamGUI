...
namespace DreamWidgetPlacementTestLocal
{
	struct FScopedWorld
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamWidgetPlacementRoundTripTest,
	"DreamGUI.Widget.Placement.ASlotSurvivesBeingLiftedOutAndPutBack",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamWidgetPlacementRoundTripTest::RunTest(const FString& Parameters)
{
	using namespace DreamWidgetPlacementTestLocal;
	FScopedWorld TestWorld;
	UDreamWidget* Root = MakeWidget(TestWorld.World, nullptr, TEXT("Root"));
	UDreamWidget* Hand = MakeWidget(TestWorld.World, Root, TEXT("Hand"));
	Hand->CreateNewLayoutContainer<UDreamLayoutContainerVerticalBox>();
	UDreamWidget* DragLayer = MakeWidget(TestWorld.World, Root, TEXT("DragLayer"));

	UDreamWidget* Filler = MakeWidget(TestWorld.World, Hand, TEXT("Filler"));
	UDreamWidget* Card = MakeWidget(TestWorld.World, Hand, TEXT("Card"));
	UDreamWidget* Trailing = MakeWidget(TestWorld.World, Hand, TEXT("Trailing"));
	...
	UDreamPanelSlot* Restored = Card->GetPanelSlot();
	...
	TestEqual(TEXT("Horizontal alignment restored"), Restored->HorizontalAlignment, EDreamPanelHorizontalAlignment::Right);
	TestEqual(TEXT("Vertical alignment restored"), Restored->VerticalAlignment, EDreamPanelVerticalAlignment::Top);
	TestEqual(TEXT("Size rule restored"), Restored->SizeRule, EDreamPanelSizeRule::Fill);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamWidgetPlacementRefusalTest,
	"DreamGUI.Widget.Placement.ARestoreWithNowhereToGoRefuses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamWidgetPlacementRefusalTest::RunTest(const FString& Parameters)
{
	using namespace DreamWidgetPlacementTestLocal;
	FScopedWorld TestWorld;
	UDreamWidget* Root = MakeWidget(TestWorld.World, nullptr, TEXT("Root"));
	UDreamWidget* Owner = MakeWidget(TestWorld.World, Root, TEXT("Owner"));
	UDreamWidget* DragLayer = MakeWidget(TestWorld.World, Root, TEXT("DragLayer"));
	UDreamWidget* Card = MakeWidget(TestWorld.World, Owner, TEXT("Card"));

	FDreamWidgetPlacement Empty;
	...
	FDreamWidgetPlacement FromRoot;
