 ...
 *
 * The arrangement now records into an FDreamFragment and the base class commits it afterwards - Blink's
...
namespace DreamLayoutFragmentTestLocal
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamFragmentArrangeWritesNothingTest,
	"DreamGUI.Layout.Fragment.ArrangeWritesNothingUntilCommit",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamFragmentArrangeWritesNothingTest::RunTest(const FString& Parameters)
{
	using namespace DreamLayoutFragmentTestLocal;
	...
	UDreamWidget* ChildA = MakeChild(TestWorld.World, Root, 30.0f, 40.0f);
	UDreamWidget* ChildB = MakeChild(TestWorld.World, Root, 50.0f, 60.0f);

	UDreamArrangeObservingOverlay* Overlay = Cast<UDreamArrangeObservingOverlay>(
		Root->CreateNewLayoutContainer(UDreamArrangeObservingOverlay::StaticClass()));
	...
	ChildA->OnRegister();
	ChildB->OnRegister();
	for (UDreamWidget* Child : { ChildA, ChildB })
	{
		if (UDreamPanelSlot* Slot = Child->GetPanelSlot())
		{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamFragmentArrangeIsQueryableTest,
	"DreamGUI.Layout.Fragment.ArrangeCanBeAskedWithoutCommitting",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamFragmentArrangeIsQueryableTest::RunTest(const FString& Parameters)
{
	using namespace DreamLayoutFragmentTestLocal;
	FScopedTestWorld TestWorld;
	...
	const FDreamFragment Fragment = Overlay->Arrange();
