 ...
 * cursor, the reason UDreamWidget::Cursor could ship with no caller anywhere, and the reason the drop
...
namespace DreamInteractionTestLocal
{
	struct FScopedWorld
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamInteractionDragSourceTest,
	"DreamGUI.Interaction.Pointer.TheDraggedWidgetStopsBlockingWhatIsUnderIt",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamInteractionDragSourceTest::RunTest(const FString& Parameters)
{
	using namespace DreamInteractionTestLocal;
	...
	UDreamWidget* Card = MakeWidget(TestWorld.World, Root, TEXT("Card"));
	UDreamWidget* CardArt = MakeWidget(TestWorld.World, Card, TEXT("CardArt"));
	UDreamWidget* Slot = MakeWidget(TestWorld.World, Root, TEXT("Slot"));

	TestTrue(TEXT("The dragged widget itself is ignored"),
		DreamPointerPolicy::ShouldIgnoreHitWhileDragging(Card, Card, true));
	...
	TestTrue(TEXT("A descendant of the dragged widget is ignored"),
		DreamPointerPolicy::ShouldIgnoreHitWhileDragging(CardArt, Card, true));
	TestFalse(TEXT("The drop target underneath is NOT ignored"),
		DreamPointerPolicy::ShouldIgnoreHitWhileDragging(Slot, Card, true));
	...
	TestFalse(TEXT("An ancestor of the dragged widget is not ignored"),
		DreamPointerPolicy::ShouldIgnoreHitWhileDragging(Root, Card, true));
	...
	TestFalse(TEXT("Not dragging: the same widget is not ignored"),
		DreamPointerPolicy::ShouldIgnoreHitWhileDragging(Card, Card, false));
	TestFalse(TEXT("No drag widget: nothing is ignored"),
		DreamPointerPolicy::ShouldIgnoreHitWhileDragging(Card, nullptr, true));
	TestFalse(TEXT("A null hit is not ignored"),
		DreamPointerPolicy::ShouldIgnoreHitWhileDragging(nullptr, Card, true));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamInteractionCursorTest,
	"DreamGUI.Interaction.Pointer.TheInnermostWidgetClaimingACursorWins",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamInteractionCursorTest::RunTest(const FString& Parameters)
{
	using namespace DreamInteractionTestLocal;
	FScopedWorld TestWorld;
	UDreamWidget* Root = MakeWidget(TestWorld.World, nullptr, TEXT("Root"), 800.0f, 600.0f);
	UDreamWidget* Panel = MakeWidget(TestWorld.World, Root, TEXT("Panel"));
	UDreamWidget* Button = MakeWidget(TestWorld.World, Panel, TEXT("Button"));

	EMouseCursor::Type Cursor = EMouseCursor::None;
	UDreamWidget* Stack[] = { Button, Panel, Root };
	...
	TestFalse(TEXT("An unclaimed stack resolves to nothing"),
		DreamPointerPolicy::ResolveCursor(Stack, Cursor));

	Button->SetCursor(EMouseCursor::Hand);
	if (TestTrue(TEXT("A claim is found"), DreamPointerPolicy::ResolveCursor(Stack, Cursor)))
	{
	...
	if (TestTrue(TEXT("A claim is still found"), DreamPointerPolicy::ResolveCursor(Stack, Cursor)))
	{
	...
	if (TestTrue(TEXT("The outer claim is reached"), DreamPointerPolicy::ResolveCursor(Stack, Cursor)))
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamInteractionInteractableTest,
	"DreamGUI.Interaction.Selectable.DisablingAnAncestorDisablesTheSubtree",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamInteractionInteractableTest::RunTest(const FString& Parameters)
{
	using namespace DreamInteractionTestLocal;
	...
	UDreamWidget* Panel = MakeWidget(TestWorld.World, Root, TEXT("Panel"));
	UDreamWidget* Button = MakeWidget(TestWorld.World, Panel, TEXT("Button"));
	UDreamWidget* Escapee = MakeWidget(TestWorld.World, Panel, TEXT("Escapee"));
	...
	Panel->SetInteractable(EDreamWidgetInteractableType::Disabled);
	...
	Escapee->SetInteractable(EDreamWidgetInteractableType::Enabled);
	...
	Panel->SetInteractable(EDreamWidgetInteractableType::Inherit);
	TestTrue(TEXT("Restoring the ancestor restores the subtree"), Button->GetInteractableInHierarchy());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamInteractionToggleGroupTest,
	"DreamGUI.Interaction.Toggle.AGroupKeepsExactlyOneOn",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamInteractionToggleGroupTest::RunTest(const FString& Parameters)
{
	using namespace DreamInteractionTestLocal;
	FScopedWorld TestWorld;
	UDreamWidget* Root = MakeWidget(TestWorld.World, nullptr, TEXT("Root"), 800.0f, 600.0f);
	UDreamWidget* GroupWidget = MakeWidget(TestWorld.World, Root, TEXT("Group"));
	...
	{
		UDreamWidget* W = MakeWidget(TestWorld.World, GroupWidget, *FString::Printf(TEXT("Toggle%d"), Index));
