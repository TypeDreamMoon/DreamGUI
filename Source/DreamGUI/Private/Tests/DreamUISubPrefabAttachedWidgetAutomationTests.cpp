...
// (EDreamUIPrefabVersion::ActorAttachToSubPrefab) must materialize on every
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamUISubPrefabAttachedWidgetBothLoadPathsTest,
	"DreamGUI.Prefab.SubPrefabAttachedWidgetBothLoadPaths",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

namespace DreamUISubPrefabAttachedWidgetTestLocal
{
	...
	UDreamWidget* FindByDisplayName(UDreamWidget* Root, const TCHAR* DisplayName)
	{
		if (!IsValid(Root))
		{
			return nullptr;
...
bool FDreamUISubPrefabAttachedWidgetBothLoadPathsTest::RunTest(const FString& Parameters)
{
	using namespace DreamUIPrefabSystem;
	using namespace DreamUISubPrefabAttachedWidgetTestLocal;
	...
	UDreamWidget* ChildSourceRoot = NewObject<UDreamWidget>(AuthorWorld, NAME_None, RF_Public | RF_Transactional);
	ChildSourceRoot->SetDisplayName(TEXT("ChildRoot"));
	ChildSourceRoot->CreateNewLayoutContainer<UDreamLayoutContainerVerticalBox>();
	UDreamWidget* ChildAttachTarget = NewObject<UDreamWidget>(AuthorWorld, NAME_None, RF_Public | RF_Transactional);
	ChildAttachTarget->SetDisplayName(AttachTargetName);
	ChildAttachTarget->CreateNewLayoutContainer<UDreamLayoutContainerVerticalBox>();
	...
	UDreamWidget* ParentRoot = NewObject<UDreamWidget>(AuthorWorld, NAME_None, RF_Public | RF_Transactional);
	ParentRoot->SetDisplayName(TEXT("ParentRoot"));
	ParentRoot->CreateNewLayoutContainer<UDreamLayoutContainerVerticalBox>();
	...
	UDreamWidget* NestedAttachTarget = FindByDisplayName(NestedRoot, AttachTargetName);
	...
	UDreamWidget* AttachedWidget = NewObject<UDreamWidget>(AuthorWorld, NAME_None, RF_Public | RF_Transactional);
	AttachedWidget->SetDisplayName(AttachedWidgetName);
	AttachedWidget->CreateNewLayoutContainer<UDreamLayoutContainerScrollBox>();
	TestTrue(TEXT("New widget attaches under the sub-prefab widget"),
		AttachedWidget->TrySetParent(NestedAttachTarget, false));
	UDreamWidget* AttachedGrandchild = NewObject<UDreamWidget>(AuthorWorld, NAME_None, RF_Public | RF_Transactional);
	...
	auto VerifyHierarchy = [&](UDreamWidget* Root, const TCHAR* PathTag)
	{
		UDreamWidget* Target = FindByDisplayName(Root, AttachTargetName);
		UDreamWidget* Attached = FindByDisplayName(Root, AttachedWidgetName);
		UDreamWidget* Grandchild = FindByDisplayName(Root, AttachedGrandchildName);
	...
	if (RuntimeWorld)
	{
		UDreamWidget* RuntimeRoot = WidgetSerializer::LoadPrefab(
	...
	TMap<TObjectPtr<UDreamWidget>, FDreamUISubPrefabData> EditorSubPrefabs;
	if (EditorWorld)
	{
		UDreamWidget* EditorRoot = WidgetSerializer::LoadPrefabWithExistingObjects(
		...
		UDreamWidget* RefreshedRoot = WidgetSerializer::LoadPrefabWithExistingObjects(
	...
	UDreamUIPrefabHelperObject* Helper = ParentPrefab->GetPrefabHelperObject();
	...
	if (Helper)
	{
		...
		if (VerifyWorld)
		{
			UDreamWidget* ResavedRoot = WidgetSerializer::LoadPrefab(
	...
	if (UDreamUIPrefabHelperObject* ChildHelper = ChildPrefab->GetPrefabHelperObject())
	{
