 ...
 *
 * These are deliberately matrix-only -- no world, no UDreamCanvas, no widget. The canvas resizes its
...
namespace DreamCanvasViewFitTestLocal
{
	...
	// How UDreamCanvas builds its own: DreamCanvas.cpp:2170-2199 for the product, :2126-2141 for the
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamCanvasViewFitPlaneKeepsPixelsTest,
	"DreamGUI.Canvas.ViewFit.TheCanvasPlaneKeepsExactlyThePixelsItHadBefore",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamCanvasViewFitPlaneKeepsPixelsTest::RunTest(const FString& Parameters)
{
	using namespace DreamCanvasViewFitTestLocal;
	...
	FMatrix Correction;
	if (!TestTrue(TEXT("A correction exists"), DreamCanvasViewFit::BuildClipCorrection(
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamCanvasViewFitForeshortensTest,
	"DreamGUI.Canvas.ViewFit.DepthOffThePlaneStillForeshortens",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamCanvasViewFitForeshortensTest::RunTest(const FString& Parameters)
{
	using namespace DreamCanvasViewFitTestLocal;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamCanvasViewFitZoomPanTest,
	"DreamGUI.Canvas.ViewFit.ZoomAndPanKeepTrackingTheOrthographicView",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamCanvasViewFitZoomPanTest::RunTest(const FString& Parameters)
{
	using namespace DreamCanvasViewFitTestLocal;
	...
	auto AgreesWithOrtho = [&](const FFixture& InFix, const TCHAR* What)
	{
		...
		if (!TestTrue(*FString::Printf(TEXT("%s: a correction exists"), What),
			DreamCanvasViewFit::BuildClipCorrection(InFix.Canvas(), InFix.Ortho(), InFix.PlaneOrigin,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FDreamCanvasViewFitRefusalTest,
	"DreamGUI.Canvas.ViewFit.ImpossibleFitsAreRefusedRatherThanApproximated",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDreamCanvasViewFitRefusalTest::RunTest(const FString& Parameters)
{
	using namespace DreamCanvasViewFitTestLocal;
	FFixture Fix;
	FMatrix Correction;
	...
	TestTrue(TEXT("An ordinary canvas is usable"),
		DreamCanvasViewFit::IsCanvasViewUsable(Fix.EyeDistance(), 1.0, 10000.0));
	TestFalse(TEXT("A canvas past its own far plane is refused"),
		DreamCanvasViewFit::IsCanvasViewUsable(12000.0, 1.0, 10000.0));
	TestFalse(TEXT("A canvas exactly on the far plane is refused"),
		DreamCanvasViewFit::IsCanvasViewUsable(10000.0, 1.0, 10000.0));
	TestFalse(TEXT("A canvas inside its own near plane is refused"),
		DreamCanvasViewFit::IsCanvasViewUsable(0.5, 1.0, 10000.0));
	TestFalse(TEXT("A canvas exactly on the near plane is refused"),
		DreamCanvasViewFit::IsCanvasViewUsable(1.0, 1.0, 10000.0));

	// A degenerate basis has no plane to pin.
	TestFalse(TEXT("A zero sample length is refused"), DreamCanvasViewFit::BuildClipCorrection(
		Fix.Canvas(), Fix.Ortho(), Fix.PlaneOrigin, FVector::YAxisVector, FVector::ZAxisVector, 0.0, Correction));
	TestFalse(TEXT("A zero basis vector is refused"), DreamCanvasViewFit::BuildClipCorrection(
	...
	TestFalse(TEXT("An eye aimed off the plane normal is refused, not approximated"),
		DreamCanvasViewFit::BuildClipCorrection(Tilted, Fix.Ortho(), Fix.PlaneOrigin,
	...
	TestFalse(TEXT("A plane behind the canvas eye is refused"), DreamCanvasViewFit::BuildClipCorrection(
