...
#define LOCTEXT_NAMESPACE "DreamUIUtils"

UTexture2D* FDreamUIUtils::CreateTexture(int32 InSize, FColor InDefaultColor, UObject* InOuter, FName InDefaultName)
{
...
TArray<uint8> FDreamUIUtils::GetMD5(uint8* InData, uint64 InSize)
{
	FMD5 Md5Gen;
...
FString FDreamUIUtils::GetMD5String(const TArray<uint8>& InMD5Digits)
{
	FString Md5String;
...
#if WITH_EDITOR
void FDreamUIUtils::NotifyPropertyChanged(UObject* Object, FProperty* Property)
{
	if (!IsValid(Object))
	{
		UE_LOG(DreamGUI, Error, TEXT("[%s].%d InValid object!"), ANSI_TO_TCHAR(__FUNCTION__), __LINE__);
		FDebug::DumpStackTraceToLog(ELogVerbosity::Warning);
		return;
	}
	if (Property == nullptr)
	{
		UE_LOG(DreamGUI, Error, TEXT("[%s].%d InValid property!"), ANSI_TO_TCHAR(__FUNCTION__), __LINE__);
		FDebug::DumpStackTraceToLog(ELogVerbosity::Warning);
		return;
	}
...
void FDreamUIUtils::NotifyPropertyChanged(UObject* Object, FName PropertyName)
{
	auto Property = FindFProperty<FProperty>(Object->GetClass(), PropertyName);
	NotifyPropertyChanged(Object, Property);
}

void FDreamUIUtils::ChangePropertyWithNotify(UObject* Object, FProperty* Property,
...
void FDreamUIUtils::ChangePropertyWithNotify(UObject* Object, FName PropertyName, TFunctionRef<void()> ChangePropertyFunction)
{
	auto Property = FindFProperty<FProperty>(Object->GetClass(), PropertyName);
	ChangePropertyWithNotify(Object, Property, ChangePropertyFunction);
}

void FDreamUIUtils::NotifyPropertyPreChange(UObject* Object, FProperty* Property)
{
	if (!IsValid(Object))
	{
		UE_LOG(DreamGUI, Error, TEXT("[%s].%d InValid object!"), ANSI_TO_TCHAR(__FUNCTION__), __LINE__);
...
void FDreamUIUtils::NotifyPropertyPreChange(UObject* Object, FName PropertyName)
{
	auto Property = FindFProperty<FProperty>(Object->GetClass(), PropertyName);
...
FColor FDreamUIUtils::ColorHSVDataToColorRGB(const FVector& InHSVColor)
{
	FLinearColor colorHSV(InHSVColor);
	return colorHSV.HSVToLinearRGB().ToFColor(false);
}
FVector FDreamUIUtils::ColorRGBToColorHSVData(const FColor& InRGBColor)
{
...
FColor FDreamUIUtils::MultiplyColor(FColor A, FColor B)
{
	FColor result;
...
void FDreamUIUtils::EditorNotification(const FText& NotifyText, bool bSuccessOrFailureSound, float ExpireDuration)
{
...
void FDreamUIUtils::LogObjectFlags(UObject* obj)
{
	UE_LOG(DreamGUI, Log, TEXT("object:%s\
\n	flagValue:%d\
\n	RF_Public:%d\
\n	RF_Standalone:%d\
\n	RF_MarkAsNative:%d\
...
void FDreamUIUtils::LogClassFlags(UClass* cls)
{
	UE_LOG(DreamGUI, Log, TEXT("class:%s\
\n	flagValue:%d\
\n	CLASS_Abstract:%d\
...
void FDreamUIUtils::StaticMeshToDreamUIMeshRenderData(const UStaticMesh* InMesh, TArray<FDreamUIMeshVertex>& OutVerts, TArray<uint16>& OutIndexes)
{
	...
	// Populate Vertex Data
	{
		...
		{
			auto WarningText = FText::Format(LOCTEXT("StaticMeshHasTooManyUVSets", "StaticMesh {0} has {1} UV sets; DreamGUI vertex data supports at most {2}."), FText::FromString(InMesh->GetName()), TexCoordsPerVertex, MAX_SUPPORTED_UV_SETS);
#if WITH_EDITOR
			FDreamUIUtils::EditorNotification(WarningText, false, 10);
#endif
			UE_LOG(DreamGUI, Warning, TEXT("[%s].%d %s"), ANSI_TO_TCHAR(__FUNCTION__), __LINE__, *WarningText.ToString());
		...
		for (uint32 i = 0; i < NumVerts; ++i)
		{
			...
			OutVerts.Add(FDreamUIMeshVertex(
				Position,
				TangentX,
				TangentZ,
				Color,
