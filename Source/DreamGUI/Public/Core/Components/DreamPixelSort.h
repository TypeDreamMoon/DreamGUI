 ...
 */
UENUM(BlueprintType)
enum class EDreamPixelSortInterval : uint8
{
...
UENUM(BlueprintType)
enum class EDreamPixelSortKey : uint8
{
 ...
 */
struct DREAMGUI_API FDreamPixelSortRunRules
{
	EDreamPixelSortInterval Interval = EDreamPixelSortInterval::Threshold;
 ...
 *
 * Everything here is mirrored by DreamUIPostProcessPixelSort.usf and the two must agree. They cannot be
...
namespace DreamPixelSort
{
	...
	DREAMGUI_API FVector2f ResolveBand(float InFirst, float InSecond);
	 ...
	 */
	DREAMGUI_API int32 ResolvePassCount(float InStrength, int32 InMaxPasses);

	/** The sort key for one colour. Mirrored by ComputeKey in the shader. */
	DREAMGUI_API float ComputeKey(const FLinearColor& InColor, EDreamPixelSortKey InKey);

	/** Whether a key takes part in the sort at all. Out-of-band pixels are the walls between runs. */
	DREAMGUI_API bool IsInBand(float InKey, const FVector2f& InBand);

	/** A stable 32-bit mix, so runs and randomness are identical on the CPU and in the shader. */
	DREAMGUI_API uint32 Hash(uint32 InValue);
	 ...
	 */
	DREAMGUI_API bool IsSortable(float InKey, int32 InIndex, const FDreamPixelSortRunRules& InRules);
	 ...
	 */
	DREAMGUI_API bool ShouldExchange(float InLowerKey, float InUpperKey, bool bInDescending);
	 ...
	 */
	DREAMGUI_API int32 ComputeDestination(const TArray<float>& InKeys, int32 InIndex,
		const FDreamPixelSortRunRules& InRules, bool bInDescending, int32 InSearchRadius);

	DREAMGUI_API FIntPoint ResolveRegionSize(bool bInUseFullSize, const FVector2f& InRectSize, const FIntPoint& InScreenSize);
}
...
class DREAMGUI_API UDreamPixelSort : public UDreamVisualPostProcess
{
	GENERATED_BODY()

public:
	UDreamPixelSort(const FObjectInitializer& ObjectInitializer);
...
protected:
	...
	UPROPERTY(EditAnywhere, Category = "DreamGUI", meta = (ClampMin = "2", ClampMax = "512",
		EditCondition = "IntervalMode == EDreamPixelSortInterval::Random || IntervalMode == EDreamPixelSortInterval::Waves"))
		int32 IntervalLength = 32;
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = "DreamGUI", meta = (ClampMin = "1", ClampMax = "512"))
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = "DreamGUI", meta = (ClampMin = "0.0", ClampMax = "1.0",
		EditCondition = "IntervalMode == EDreamPixelSortInterval::Threshold"))
...
public:
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
		EDreamPixelSortAxis GetSortAxis()const { return SortAxis; }
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
		EDreamPixelSortKey GetSortKey()const { return SortKey; }
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
		EDreamPixelSortInterval GetIntervalMode()const { return IntervalMode; }
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
	...
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
		void SetSortAxis(EDreamPixelSortAxis Value);
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
		void SetSortKey(EDreamPixelSortKey Value);
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
		void SetIntervalMode(EDreamPixelSortInterval Value);
	UFUNCTION(BlueprintCallable, Category = "DreamGUI")
