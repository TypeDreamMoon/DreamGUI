 ...
 *
 * Both were previously inline in UDreamPointerInputModule, which is only reachable with a world, a
...
namespace DreamPointerPolicy
{
	 ...
	 */
	DREAMGUI_API bool ShouldIgnoreHitWhileDragging(const UDreamWidget* InHitWidget, const UDreamWidget* InDragWidget, bool bInIsDragging);

	/**
	 * The cursor to show for a hover stack, innermost widget first.
	 *
	 * UDreamWidget::Cursor has existed, been editable in the details panel, and done absolutely
	 ...
	 */
	DREAMGUI_API bool ResolveCursor(TArrayView<UDreamWidget* const> InHoverStackInnermostFirst, EMouseCursor::Type& OutCursor);
}
